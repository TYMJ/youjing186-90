...
# 转换常量
PX_PER_PT = 1.333       # 1磅 ≈ 1.333像素
PX_PER_UNIT = 7.2       # Excel标准列宽单位约等于 7.2 像素

def get_cell_size_px(ws, row, col):
    """获取单元格的近似像素宽高"""
    letter = get_column_letter(col)
    # 获取列宽（默认8.43）
    cw = ws.column_dimensions[letter].width or 8.43
    # 获取行高（默认15磅）
    rh = ws.row_dimensions[row].height or 15
    return cw * PX_PER_UNIT, rh * PX_PER_PT

def insert_image_centered(ws, img_path, row, col):
    """
    仿照 report.py 的 offset_img 逻辑
    并加入自动等比例缩放与居中算法
    """
    ...
    # 1. 计算缩放后的图片尺寸 (等比例缩放)
    cell_w, cell_h = get_cell_size_px(ws, row, col)
    with PILImage.open(img_path) as pil_img:
        orig_w, orig_h = pil_img.size

    # 计算缩放比 (取宽比和高比的最小值，确保图片完全在格内)
    ratio = min(cell_w / orig_w, cell_h / orig_h) * 0.9  # 乘0.9留一点白边
    new_w = int(orig_w * ratio)
    new_h = int(orig_h * ratio)

    # 2. 计算居中偏移量 (像素)
    off_x = int((cell_w - new_w) / 2)
    off_y = int((cell_h - new_h) / 2)

    # 3. 使用 report.py 的 Anchor 逻辑进行插入
    img = OpenpyxlImage(img_path)
    p2e = pixels_to_EMU
    ...
    # 定义锚点位置及精确偏移 (基于 report.py 的 AnchorMarker)
    marker = AnchorMarker(
        col=col-1, 
        colOff=p2e(off_x), 
        row=row-1, 
...
@require_token
async def api_custom_export_optimized(request):
    """优化后的自定义报表导出 API"""
    s = Session()
    try:
        j = await request.json()
        ...
        if not rid:
            return {"code": -1, "msg": "未指定报表"}

        # 获取用户权限和模板 (还原 Delphi 逻辑)
        user_rec = s.query(sys_user).filter(
            sys_users.username == user_name,
            or_(
                sys_users.path.like('%优景外销%'),
                sys_users.username == 'zjnblh',
                sys_users.username == '侯柳红'
            )
        ).first()
        if not user_rec:
            return {"code": -1, "msg": "用户权限不足，无法导出"}

       
        #  获取模板路径
        cyzglsheet_rid = s.query(cyzglsheet.rid).filter(
            cyzglsheet.zm == "报价自定义模板",
            cyzglsheet.xm == user_name
        ).first()
        
        if not cyzglsheet_rid:
            return {'code': -1, 'msg': f'未在 cyzglsheet 中找到模板 "{zm}" 的配置'}

        # B. 从 sys_attachment 查找真正的文件路径
        r_path = s.query(sys_attachment.path).filter(
            sys_attachment.pid == cyzglsheet_rid,
            sys_attachment.path != None
        ).first() # 取最新的一个
        if not r_path:
            return {'code': -1, 'msg': '模板文件在附件表中不存在'}
        wb = load_workbook(r_path)
        ws = wb.active
        
        # 2. 建立表头映射 {列索引: 字段名}
        head_map = {c: ws.cell(1, c).value for c in range(1, ws.max_column + 1) if ws.cell(1, c).value}

        # 3. 获取数据并填充

        products = s.query(bjsheet).filter(bjsheet.pid==rid).all()
        for i, prod in enumerate(products):
            row_idx = i + 2  # 从第二行开始
            
            for col_idx, head in head_map.items():
                if head == "图片":
                    # 获取图片路径
                    img_name = getattr(prod, "tphh", "")
                    img_path = os.path.join(config.image_path, f"{img_name}.jpg")
                    # 调用优化后的居中插入函数
                    insert_image_centered(ws, img_path, row_idx, col_idx)
                else:
                    val = getattr(prod, head, "")
                    ws.cell(row=row_idx, column=col_idx).value = val

        # 4. 保存并返回
        out_filename = f"Export_{get_uuid()}.xlsx"
        wb.save(os.path.join(config.upload_path, out_filename))
        return {"code": 1, "msg": "导出成功", "path": out_filename}
