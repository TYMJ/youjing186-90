    ...
    try:
        ...
        if len(d)==0:
            ...
            if sbcx!='' and sbcx!=None:
                logger.error('==2==')
                logger.error(d[0].get('bz',''))
                logger.error(str(sbcx))
...
async def view_saier_purchase_process_batch_cgrk_btn(request):
    ...
    try:
                    ...
                    if jczsl <= htzsl:
                        ...
                        if (ckmc == '义乌仓库') or (ckmc != '宁波志恒') or (ckmc != '汕头仓库') or (ckmc != '宁波龙和') or (ckmc != '宁波万纬'):
                            ...
                            if index == 1:
                                ...
                                m.jcrq = jcrq
                                m.Operator = Operator1
                                m.SalesOrderNo = d.wxht
                                m.Exporter = d.wfgs
                                m.Salesman = d.ywry
                                m.PurchaseOrderNo = d.hthm
                                m.SupplierShortName = d.sccj
                                m.PurchasingAgent = d.cgry
                                m.WarehouseName = ckmc
                                m.SNID = jcbh
                                m.sfjc = '否'
                                m.gdry = d.gdry
                            ...
                            m.ctime = time.strftime('%Y-%m-%d %H:%M:%S')
                            m.wyzd = jc_rid
                            m.OuterLength = l.bzcd
                            m.OuterWidth = l.bzkd
                            m.OuterHeight = l.bzgd
                            m.ItemNo = l.cpbh
                            m.OuterGrossWeight = l.mz
                            m.OuterNetWeight = l.jz
                            m.ExpectedCartonQty = l.jcxs
                            m.OuterVolume = l.wxcc
                            m.Volume = l.htztj
                            m.GrossWeight = l.htzmz
                            m.zwmc = l.zwmc
                            m.wxwyzd = l.wxwyzd
                            m.cgwyzd = l.wyzd
                            m.ywpath = ywpath
        ...
        if sl_flag == 1:
            return json_result(-1, f"合同的进仓数量{jczsl}小于合同数量{htzsl}，请核对后再进行批量入库单的操作!")
        if check == 0:
