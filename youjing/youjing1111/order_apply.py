...
async def order_wfgs_kind(wfgs_n,khmc,ywbm):
    try:
        htlx = ''
...
async def order_khmc_kind(khmc,ywbm,ywy):
    try:
        htlx = ''
        bmdm = ''
        year = time.strftime('%Y')[2:4]
...
async def view_saier_order_apply_wfgs_change(request):
    ...
    try:
        ...
        khmc = j.get('khmc', '')
        ywbm = j.get('ywbm', '')
        htlx, bmdm = await order_wfgs_kind(wfgs_n,khmc,ywbm)
...
async def view_saier_order_apply_khmc_change(request):
    ...
    try:
        ...
        ywbm = j.get('ywbm', '')
        ywy = j.get('ywy', '')
        htlx, bmdm = await order_khmc_kind(khmc,ywbm,ywy)
...
async def order_get_no(wfgs_n,bmdm,mdck,ckyw,htlx,khmc,user,s,flag=0):
    try:
        hthm = ''
        number = 1
        # flag = 0
...
async def view_saier_order_apply_save_before(request):
    ...
    try:
        ...
        ckmc = j.get('mdka', '')
        ckyw = j.get('ckyw', '')
        hthm,path = await order_get_no(wfgs_n,bmdm,ckmc,ckyw,htlx,khmc,user,s,1)
...
async def view_saier_order_apply_load_check(request):
    ...
    try:
        ...
        #         if len(c) > 0:
        #             # bmdm= c[0].get('dyywzm','')
        #             htlx = str(time.strftime('%Y')[:2]) + str(c[0].get('htbh','')) + str(time.strftime('%m'))
        # else:
        ...
        if (zsht!='' and zsht!=None and path2 != '' and  path1 in path2) or (ywy!='' and ywy != None and user.username == ywy):
            ...
            fkqd_list = run_sql(f"SELECT '{rid}' pid,cght,htje,sqje,fkrq,ifnull(fkje,0) fkje,\
            ...
            d = run_sql(f"SELECT '{rid}' pid,cght,htje,sqje,hkrq fkrq,ifnull(fkhj,0) fkje,\
