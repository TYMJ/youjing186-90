...
from .__default__ import user_task_new,module_workflow_get_task,user_task_delete
...
async def view_saier_suppliers_save_before(request):
    ...
    try:
        ...
        s.commit()
        return json_result(1, '取数成功', {'bhbm': bhbm, 'bhjg': bhjg, 'gcsh': gcsh, 'xjsb': xjsb, 'xgsb': xgsb, 'kpgc':kpgc})
    except:
        s.rollback()
        logger.error(trace_error())
        return json_result(-1, trace_error(), '')
