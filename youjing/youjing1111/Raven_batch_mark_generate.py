...
def _get_template_path(mark_type, env_opt, region_opt):
    ...
    mapping = {
        ...
        '港加特标唛头': f'{prefix}港加标{suffix}.xlsx',
        '港加特标加警语唛头': f'{prefix}港加标加警{suffix}.xlsx',
    ...
    try:
        ...
        for order in selected_orders:
            ...
            for detail in details:
                ...
                _fill_by_mark_type(ws, mark_type, detail, {
                    'czkrhh': czkrhh,
                    'djpmw': djpmw,
                    'ship_date': ship_date,
                    'khht': khht,
                ...
                if tmtp:
                    _try_add_image(ws, _abs_upload(tmtp), 'A6' if mark_type in ('港加特标唛头', '港加特标加警语唛头') else 'A4')
        ...
        return json_result(code=1, msg=f'成功生成{len(files)}个唛头文件', data={
            'files': files,
            'count': len(files)
        })
