...
async def view_saier_declaration_save_before(request):
    ...
    try:
                        ...
                        if sz != 0:
                            ...
                            if rmbkh == '是':
                                ...
                                if hbdm in ('USD$', 'USD'):
                                    ...
                                    d = run_sql(f"select * from bgdjjssheet where ssgs='{ssgs}' and zzsl='{zzsl}' and tsl='{tsl}' and hbdm='{hbdm}'")
        ...
        # ===== AMZ费用回写 =====
        if 'AMZ' in fphm:
            d = run_sql(f"select fyzj1,mdgfy,gnnlf,hybfM,qggsM,`qgzs$`,`mdgewfy$` from cymx where fphm='{fphm}'")
            if len(d) > 0:
                rr = d[0]
                if 'CNF' in jgtk.upper():
                    ...
                    dszfy = chhj + _f(rr.get('qggsM',0)) + _f(rr.get('qgzs$',0)) + _f(rr.get('mdgewfy$',0))
                else:
...
async def view_saier_declaration_button_cwplfj(request):
    ...
    try:
        ...
        files = []
        for r in rids:
            d = run_sql(f"select rid,fphm from bgmxd where ((tzcw='{user.username}') or (tzcw="") or (tzcw is null)) and (rid = '{r}')")
            if len(d) > 0:
...
def _kptz_upsert_fpgl_full(fphm: str, ysfp: str, user, now_dt, main_row: dict, db_session):
    ...
    bg = run_sql("""
        select rid,fphm,ysfp,khmc,RMBkh,bggs,ywry,ywbm,zdry,dlmc,zwpmz,
               ...
               ifnull(bgdh,'') as bgdh,ifnull(bglx,'') as bglx,ifnull(fpbz12,'') as fpbz12
          from bgmxd
    ...
    vals = {
        ...
        "bglx": S(b.get("bglx")), "fpbz": S(b.get("fpbz12")),
        "sfnm": "是" if S(b.get("bglx")) == "内销" else "否",
        ...
        "dfrq": S(c.get("dfrq")), "mdka": S(c.get("mdka")), "cdmc": S(c.get("cdmc")), "mygb": S(c.get("mygb")),
        "ywdz": ywdz, "ywxz": ywxz,
...
def _kptz_get_yw_tags(ywry: str, ywbm: str) -> dict:
    """
    统一补齐 ywdz / ywxz / zzsw
    优先级：ywdzb(按ywry) > ywdzb(按ywbm) > cyzglsheet配置 > 默认值
    """
    ywry = _s(ywry)
    ywbm = _s(ywbm)
    out = {"ywdz": "", "ywxz": "", "zzsw": ""}
    ...
    if ywry:
        d1 = run_sql("""
            select ifnull(ywdz,'') ywdz, ifnull(ywxz,'') ywxz, ifnull(zzsw,'') zzsw
              from ywdzb
             where ywry=:ywry
             limit 1
        """, {"ywry": ywry}) or []
    ...
    # fallback: cyzglsheet
    if not out["ywdz"] and ywry:
        t = run_sql("select ifnull(bz,'') v from cyzglsheet where xm=:xm and zm='业务大组' limit 1", {"xm": ywry})
        if t: out["ywdz"] = _s(t[0].get("v"))
    if not out["ywxz"] and ywry:
        t = run_sql("select ifnull(bz,'') v from cyzglsheet where xm=:xm and zm='业务性质' limit 1", {"xm": ywry})
        if t: out["ywxz"] = _s(t[0].get("v"))
    if not out["zzsw"] and ywry:
        t = run_sql("select ifnull(bz,'') v from cyzglsheet where xm=:xm and zm='征税属性' limit 1", {"xm": ywry})
        if t: out["zzsw"] = _s(t[0].get("v"))
...
async def view_saier_declaration_button_kptz_run_v2(request):
    ...
    ysfp_in = _s(j.get('ysfp'))
    ywrya_in = _s(j.get('ywrya'))
    if not rid:
    ...
    try:
        ...
        for x in ur:
            ...
            rr = run_sql("""
                select rid from sys_role
                 where rid=:rid and name like '%财务%' and path like '%优景%'
                 limit 1
            """, {"rid": role_id})
            if rr:
                ok_role = True
        ...
        # 业务标签（优先 ywdzb，回退 cyzglsheet）
        yw_tags = {"ywdz": "", "ywxz": "", "zzsw": ""}
        t1 = run_sql("""
            select ifnull(ywdz,'') ywdz, ifnull(ywxz,'') ywxz, ifnull(zzsw,'') zzsw
              from ywdzb where ywry=:ywry limit 1
        """, {"ywry": _s(m.get('ywry'))}) or []
        if not t1:
            t1 = run_sql("""
                select ifnull(ywdz,'') ywdz, ifnull(ywxz,'') ywxz, ifnull(zzsw,'') zzsw
                  from ywdzb where ywbm=:ywbm limit 1
            """, {"ywbm": _s(m.get('ywbm'))}) or []
        if t1:
            yw_tags["ywdz"] = _s(t1[0].get("ywdz"))
            yw_tags["ywxz"] = _s(t1[0].get("ywxz"))
            yw_tags["zzsw"] = _s(t1[0].get("zzsw"))
        if not yw_tags["ywdz"]:
            x = run_sql("select ifnull(bz,'') v from cyzglsheet where xm=:xm and zm='业务大组' limit 1", {"xm": _s(m.get('ywry'))})
            if x: yw_tags["ywdz"] = _s(x[0].get("v"))
        if not yw_tags["ywxz"]:
            x = run_sql("select ifnull(bz,'') v from cyzglsheet where xm=:xm and zm='业务性质' limit 1", {"xm": _s(m.get('ywry'))})
            if x: yw_tags["ywxz"] = _s(x[0].get("v"))
        if not yw_tags["zzsw"]:
            x = run_sql("select ifnull(bz,'') v from cyzglsheet where xm=:xm and zm='征税属性' limit 1", {"xm": _s(m.get('ywry'))})
            if x: yw_tags["zzsw"] = _s(x[0].get("v"))
        ...
        if not sbdate_final:
            need.append("sbdate")
        if ('义乌财务' in position) and not _s(gsfprq):
            need.append("gsfprq")
        if need:
        ...
        if need_full_fpgl:
            ...
            strict = True
            cfg = run_sql("select cs from zx where ly='KPTZ_FPGL_STRICT' limit 1")
            if cfg:
                v = _s(cfg[0].get('cs')).lower()
                strict = v in ('1', 'true', 'yes', '是')

            split_rows = run_sql("""
                select ifnull(zzsw,'') zzsw,
                    ifnull(sum(sjkpze),0) sjkpze,
                    ifnull(sum(bhsje),0) bhsje,
                    ifnull(sum(se),0) se
                from kaiptzsheet1
                where fphm=:f
                group by ifnull(zzsw,'')
            """, {"f": wxfp}) or []
            ...
            for r in split_rows:
                k = _s(r.get("zzsw"))
                sj = _f(r.get("sjkpze")); bh = _f(r.get("bhsje")); se = _f(r.get("se"))
                if ('免' in k) or ('零税' in k):
                    ms_sjkpze += sj; ms_bhsje += bh; ms_se += se
                elif ('征' in k) or ('应税' in k):
                    zs_sjkpze += sj; zs_bhsje += bh; zs_se += se
                else:
                    qt_sjkpze += sj; qt_bhsje += bh; qt_se += se

            if strict:
                zs_sjkpze = math.trunc(zs_sjkpze * 100) / 100
                zs_bhsje = math.trunc(zs_bhsje * 100) / 100
                zs_se = math.trunc(zs_se * 100) / 100
                ms_sjkpze = math.trunc(ms_sjkpze * 100) / 100
                ms_bhsje = math.trunc(ms_bhsje * 100) / 100
                ms_se = math.trunc(ms_se * 100) / 100
                qt_sjkpze = math.trunc(qt_sjkpze * 100) / 100
                qt_bhsje = math.trunc(qt_bhsje * 100) / 100
                qt_se = math.trunc(qt_se * 100) / 100
        ...
        if all_new_head_rids:
            s.query(kaiptz).filter(kaiptz.rid.in_(all_new_head_rids)).update({
                "uid": user.rid, "path": gd_path, "gdry": ywrya_in, "gdpath": gd_path, "mtime": now
            }, synchronize_session=False)
