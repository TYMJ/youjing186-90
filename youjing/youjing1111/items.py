...
async def insert_script_log(module, keyNo, kind, status, user, logText,photos=None, s=None):
    ...
    try:
        ...
        m.LogText = logText
        if photos and photos!='' and photos!='[]':
            logger.error(photos)
...
async def view_saier_items_zs_load_check(request):
    ...
    try:
        ...
        if wfgs_n==None or wfgs_n=='':
            ...
            if d:
                data['wfgs'] = d.wfgs
        d = s.query(func.count(bjsheet.rid).label('qty')).filter(bjsheet.bjhh==cpbh,or_(bjsheet.bjry!=user.username,bjsheet.bjbjwyzd1==None,bjsheet.bjbjwyzd1=='')).first()
