    ...
    try:
        ...
        for r in lines:
            ...
            if sfdb == '是':
                ...
                bbwc = 0
                chslz = r.get('chsl', 0)
                d = s.query(cymxsheet.rid).filter(cymxsheet.cpbh == cpbh, cymxsheet.cywyzd ==
                            cywyzd, cymxsheet.wxwyzd == wxwyzd, cymxsheet.sfdb == '是').first()
                if not d:
                    c = s.query(bgmxdsheet.rid).filter(bgmxdsheet.cpbh == cpbh, bgmxdsheet.cywyzd ==
                                cywyzd, bgmxdsheet.wxwyzd == wxwyzd, func.ifnull(bgmxdsheet.yfph, '') == '').first()
                    if c:
                        new_data[rid] = {'msg': f"请注意此合同{hthm}已在报关明细"}
                    c = s.query(bgmxdsheet.rid).filter(bgmxdsheet.cpbh == cpbh, bgmxdsheet.cywyzd ==
                                cywyzd, bgmxdsheet.wxwyzd == wxwyzd, func.ifnull(bgmxdsheet.yfph, '') != '').all()
                    for l in c:
                        if l.bbwc == '是':
                            bbwc = 1
                        if cgdj > 0:
                    ...
                    new_data[rid] = {'sfdb': '否',
                        'msg': f"请注意此票在出运明细{d.fphm }已待报不能再选!!"}
                    sfdb = ''
    ...
    try:
        ...
        if fphm != cydh and fphm != None:
            ...
            if webpd == '是' and sjcy != '' and sjcy != None:
                flag = 0
                d = s.query(kh).filter(gdqkb.kh_id == kh_id).first()
                if d:
                    if shr != '' and shr != None:
                        d.shr = shr[:250]
                        flag = 1
...
async def api_sairer_shpipment_before_save(request):
    ...
    try:
        ...
        if len(c) > 0:
            csz = 1
            btsb = 1
            c = run_sql(f"select xybx,sz from chyjh where (wxfp='{fphm}') and (fphm='{cydh}') limit 1")
            if len(c) > 0:
                jh_data = c[0]
        c = s.query(cymx.rid).filter(cymx.fphm == fphm, cymx.rid!=rid).first()
        if c:
            return json_result(-1, '发票号码已存在,请重新输入')
        
        m = s.query(cymx.zdry,cymx.fphm).filter(cymx.rid == rid).first()
        if m :
            zdry = m.zdry
        else:
            zdry = user.username
        ...
        if cydh == fphm:
            d = s.query(cymx).filter(cymx.fphm.in_(suffixes)).first()
            if not d:
                fksb = '是'
                cysb = '是'
        else:
            for r in suffixes:
                if fphm == str(cydh) + str(r):
                    ...
                    cysb = '是'
                    break
        fphm_list = [cydh + suffix for suffix in suffixes]
        s.query(cymx).filter(cymx.fphm.in_(fphm_list)).update({cymx.fksb: '是', cymx.cysb: '是'}, synchronize_session=False)
        s.query(cymx).filter(cymx.fphm.notin_(fphm_list),cymx.chydh==cydh).update({cymx.fksb: '否', cymx.cysb: '否'}, synchronize_session=False) 
        ...
        if m: 
            ...
            if m.zdry != zdry:
                if zdry != user.username and zdry != '' and zdry != None:
                    xxnr = f'出运明细发票号为:{fphm}' + '请查看,日期:' + time.strftime('%Y-%m-%d')
                    row = {
                        "xxly": '出运明细',
                        "bjdh": '',
                        "wxht": fphm,
                ...
                if tjry != user.username and tjry != '' and tjry != None:
                    xxnr = f'出运明细发票号为:{fphm}' + '已分配给:' + str(zdry) + ',请查看,日期:' + time.strftime('%Y-%m-%d')
                    row = {
                        "xxly": '出运明细',
                        "bjdh": '',
                        "wxht": fphm,
