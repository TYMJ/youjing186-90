...
const declaration_before_save = (evt_id, recordset) => {
    return new Promise((resolve, reject) => {
        _.http.post('/api/saier/declaration/save/before', {
            ...
            if (!res || res.code === -1) {
                _.ui.message.error((res && res.msg) || '保存前处理失败');
                reject();
                return;
            }
            ...
            _.ui.message.error((err && err.msg) || '保存前处理失败');
            reject();
        });
    });
};
...
const declaration_form_BtnClick = (evt_id, btn, form) => {
    let username = _.user.username
    ...
    const customs_merge_items = (recordset) => {
        ...
        for (let row of items_data) {
            if (row.sfdb != '是'){
                if (row.zzsl > 0){
                    const newTsl = row.zzsl === 6 ? '5' : row.zzsl === 4 ? '4' : '3';
                    row.tsl = newTsl;
    ...
    if (btn.name == 'declaration_dddrmb_btn') {
        ...
        _.http.post('/api/saier/declaration/button/dddrmb/options', {rids: rids}).then(res => {
    ...
    if (btn.name === 'declaration_kptz_btn') {
        ...
        const submitKptz = (sbdateVal, gsfprqVal) => {
            return _.http.post('/api/saier/declaration/button/kptz/run_v2', {
                ...
                if (d.download_filename) {
                    _.http.download('/api/tmp/file/get', { file: d.download_filename, fn: d.download_filename });
                }
        ...
        askDate(
            ...
            const pos = (_.user && (_.user.position || _.user.postion) || '');
            const needGsfp = String(pos).indexOf('义乌财务') >= 0;

            if (!needGsfp) {
                return submitKptz(sbdateVal, recordset.val('国税发票日期') || '');
            }

            return askDate(
                '请输入国税发票日期格式2010-01-18',
                recordset.val('国税发票日期') || '',
                '国税发票日期格式不正确'
            ).then(gsfprqVal => submitKptz(sbdateVal, gsfprqVal));
        }).catch(err => {
    ...
    if (btn.name == 'INV_pack_sales_btn') 
      { let rids = form.current_rids.value
        ...
        _.http.post('/api/saier/export/detailed/contract/list', {
            rids: rids,
        }).then(res => {
            let d = res.data;
