...
const evt_rkd_Beforesave = (evt_id, recordset) => {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            var jcdh = recordset.val('进仓单号');
            var jcrq = recordset.val('进仓日期');
            var gdry = recordset.val('跟单人员');
            var ywry = recordset.val('业务员');
            var sjjc = recordset.val('入库日期');
            var sfjc = recordset.val('是否进仓');
            var khmc = recordset.val('客户名称');
            var rkrq = recordset.val('入库日期');
            var rksj = recordset.val('入库时间');
            var jcrq = recordset.val('进仓日期');
            var sdry = recordset.val('输单人员');
            var wxht = recordset.val('外销合同');
            var gsmc = recordset.val('公司名称');
            var cght = recordset.val('采购合同');
            var csmc = recordset.val('厂商名称');
            var cgy = recordset.val('采购员');
            var lhy = recordset.val('理货员');
            var ckmc = recordset.val('仓库名称');
            var yzrq = recordset.val('预装日期');
            var rkdd = recordset.val('入库地点');
            ...
            if (recordset.val('是否进仓') == '是') {
                if (recordset.val('入库日期') != '' && recordset.val('入库时间') != '') {
                    ...
                    _.http.post('/api/Ravencloud/storagezyh', {
                            "zyh": recordset.val('作 业 号'),
                            "zyh1": zyh1,
                            "ywry": ywry,
                            "name": recordset.user.username,
                            "cgy": recordset.val('采购员'),
                        ...
                        if (res.code == 1) {
                            ...
                            //     recordset.val('作 业 号', result.zyh);
                            // }
                            if (result.ggxx) {
