            ...
            fileInput.onchange = async (e) => {
                ...
                try {
                    const res = await _.http.post(
                        '/api/Ravencloud/generate_mark_labels',
                        formData, {
                            headers: {
    ...
    if (btn.name == 'tsmt') {
        (async () => {
            try {
                ...
                formData.append('filename_prefix', filenamePrefix || '特殊唛头_');
 ...
 if (btn.name == 'htpldc') {
    (async () => {
        try {
            ...
            let gsInput = await _.ui.show_input_dialog(
            ...
            let pdfInput = await _.ui.show_input_dialog(
            ...
            const res = await _.http.post('/api/Ravencloud/generate_contract_batch_image', formData, {
            ...
            if (files.length === 0) {
                _.ui.message.warning('未生成文件，请检查数据或模板');
                return;
            }
            ...
            files.forEach((filePath, index) => {
                ...
                const ext = lower.endsWith('.xlsx') ? '.xlsx' : '.pdf';
                setTimeout(() => {
                    _.http.download('/api/file/get', { file: filePath }, `采购合同图_${index + 1}${ext}`);
                }, index * 300);
            });

            if (res.data && res.data.warning_file) {
                setTimeout(() => {
                    _.http.download('/api/file/get', { file: res.data.warning_file }, '诚信报告提醒.txt');
                }, files.length * 300 + 200);
            }

            _.ui.message.success(`优景采购合同批量（图）生成完成，共${count}个文件`);
        } catch (err) {
