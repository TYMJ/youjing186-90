const evt_ckd_FieldChanged = (evt_id, opts) => {
        ...
        //     recordset.val('出库单.产品资料.在仓天数', days);
            ...
            if (rkrq && ckrq && cktj > 0) {
                ...
                if (isNaN(start.getTime()) || isNaN(end.getTime())) {
                    console.error('日期格式错误', rkrq, ckrq);
                    return;
                }

                var diffTime = end - start;
                var days = Math.floor(diffTime / (24 * 60 * 60 * 1000)) + 1;

                // 防止负数
                if (days < 0) {
                    console.warn('出库日期早于入库日期', days);
                    days = 0;
                }
    ...
    if (field.full_name == '出库单.费用生成') {
        // var newValue = recordset.val('出库单.费用生成');
        // if (newValue == '否') {
        //        return;  // 这是程序自动重置的，不处理
        //    }
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log('3')
            ...
            if (recordset.val('是否完成') != '是') {
                //需要更出库单号
                if (recordset.val('出库单号') == '') {
                    _.http.post('/api/ravencloud/generate_ckdh_ckd', {}, true)
                        .then(res => {
                            if (res.code == 1) {
                                recordset.val('出库单号', res.data);
                                resolve();
                              
                            } else {
                                _.ui.message.error(res.msg || '生成出库单号失败');
                                reject();
                            }
                        })
                        .catch(error => {
                            console.error('生成出库单号错误:', error);
                            _.ui.message.error('生成出库单号失败: ' + error.message);
                            reject();
                        });
                ...
                if (res.code == 1) {
                    ...
                    if (result.cpzlUpdates && result.cpzlUpdates.length > 0) {
                        ...
                        for (const data of result.cpzlUpdates) {
                            ...
                            if (row) {
                                if (data.wyzd) row['唯一字段'] = data.wyzd;
                                if (data.ckwyzd) row['出库唯一字段'] = data.ckwyzd;
                                if (data.zcxs !== undefined) row['在仓箱数'] = data.zcxs;
                                if (data.zcxs1 !== undefined) row['在仓箱数1'] = data.zcxs1;
                                if (data.kcxs !== undefined) row['库存箱数'] = data.kcxs;
                                if (data.qckxs !== undefined) row['前出库箱数'] = data.qckxs;
                                if (data.xh) row['箱  号'] = data.xh;
                                if (data.fh) row['封  号'] = data.fh;
                                if (data.ckrq) row['出库日期'] = data.ckrq;
...
//                 userName: recordset.user.username
//             };
//             var dataData = recordset.tables['产品资料'].view_data;

//             _.http.post('/api/Ravencloud/delivery_beforesavenew', {
//                     "dataData": dataData,
//                     "mainData": mainData
