...
async def view_saier_payment_approval_load_check(request):
    ...
    try:
        ...
        if fkbh != '' and fkbh != '待生成':
            d = run_sql(f"select rid from sys_record_lock where rid='{str(rid)}' and module='付款审批' and uid!='{str(user.rid)}'")
            if len(d)>0:
                sb = 3
...
async def view_saier_payment_approval_fphm_change(request):
    ...
    try:
        ...
        d = run_sql(f"select chrq from gchk where (wxfp='{str(fphm)}') and ((gcmc='{str(cwgc)}') or (gcmc1='{str(csmc)}')) and (chrq is not null) limit 1")
        if len(d)>0:
            chrq = d[0].get('chrq','')
...
async def view_saier_payment_approval_fkfp_cght_change(request):
    ...
    try:
        cght = j.get('cght', '')
        fktt = j.get('fktt', '')
        cght_list = j.get('cght_list', [])  # 可选：前端传入本票明细合同列表
        ...
        # if ttsb12 != '' and ttsb12 != fktt:
        #     return json_result(0, f"不好意思,此采购合同已有付款抬头为{ttsb12}请更改,谢谢!", '')
        ...
        if len(d) > 0:
            ...
            if fktt1 != fktt:
                ttsb = '1'
                # return json_result(0, f"不好意思,此采购合同已有付款抬头为{fktt1}请更改,谢谢!", '')
        ...
        if cght != '':
            ...
            if len(ds) == 0:
                ...
                if len(d) == 0:
                    ...
                    rtspdata = 2
                    rtspdata1 = {
                        'DeliveryDate': d[0].get('DeliveryDate', ''),
                        'OurCompany': d[0].get('OurCompany', ''),
                        'Purchaser': d[0].get('Purchaser', ''),
                        'TotalPurchaseAmount': d[0].get('TotalPurchaseAmount', 0),
                        'OrderDate': d[0].get('OrderDate', ''),
                        'SettlementMethod': d[0].get('SettlementMethod', ''),
                        'PurchaseCurrency': d[0].get('PurchaseCurrency', ''),
                        'Department': d[0].get('Department', ''),
                        'kpgc': d[0].get('kpgc', '')
                    }
                    father1 = d[0].get('rid', '')
            else:
                ...
                rtspdata1 = {
                    ...
                    'lxry': ds[0].get('lxry', ''),
                    'sjhm': ds[0].get('sjhm', ''),
                    'htje': ds[0].get('htje', 0),
                    'gdry': ds[0].get('gdry', ''),
                    'yhje': ds[0].get('yhje', 0),
                    'htrq': ds[0].get('htrq', ''),
                    'jsfs': ds[0].get('jsfs', ''),
                    'hbdm': ds[0].get('hbdm', ''),
                    'gdbm': ds[0].get('gdbm', ''),
            ...
            if father1 != '':
                ...
                if len(d) > 0:
                    for r in d:
                        ...
                        cpbh = r.get('ProductNo', '') if cpbh == '' else cpbh + "\r\n" + r.get('ProductNo', '')
                        zsl = r.get('Quantity', '') + r.get('UnitOfMeasurement', '') if zsl == '' else zsl + "\r\n" + r.get('Quantity', '') + r.get('UnitOfMeasurement', '')
                        zwpm = r.get('ProductName', '') if zwpm == '' else zwpm + "\r\n" + r.get('ProductName', '')
            ...
            if father != '':
                ...
                if len(d) > 0:
                    for r in d:
                        if r.get('yjcq', '') != '':
                            yjch1 = r.get('yjcq', '')
                        yjcq = r.get('yjcq', '') if yjcq == '' else yjcq + "\r\n" + r.get('yjcq', '')
                        if r.get('zzsl', 0) > 0:
                            zzsl = str(r.get('zzsl', 0))
                        cpbh = r.get('bjhh', '') if cpbh == '' else cpbh + "\r\n" + r.get('bjhh', '')
                        zsl = str(r.get('cgsl', '')) + r.get('jldw', '') if zsl == '' else zsl + "\r\n" + r.get('cgsl', '') + r.get('jldw', '')
                        zwpm = r.get('zwpm', '') if zwpm == '' else zwpm + "\r\n" + r.get('zwpm', '')
        ...
        sfkp = '是' if str(zzsl) not in ('', '0', 0) else '否'

        dup = 0
        if isinstance(cght_list, list) and cght != '':
            if sum(1 for x in cght_list if x == cght) > 1:
                dup = 1
...
async def view_saier_payment_approval_zcfkxq_sb_change(request):
    ...
    try:
        ...
        if len(d)>0:
            sb1 = '本地'
            gchk_rid = d[0].get('rid', '')
            ds = run_sql(f"select * from gchksheet where pid='{str(gchk_rid)}'")
            if len(ds)>0:
                for r in ds:
                    zwpm_val = r.get('zwpm', '')
        ...
        if ysqje1 == 0:
            d = run_sql(f"select ifnull(sum(seje), 0) as seje1 from fkspsheet4 where sb='{str(sb)}'")
            if len(d)>0:
            ...
            for r in d:
                ...
                if len(ds)>0:
                    if ds[0].get('bm','') != '':
                        ...
                        s.query(fksp).filter(fksp.rid == str(r.get('rid',''))).update(update_json)
                        flag = True
...
async def view_saier_payment_approval_button_sxcwjsq(request):
    ...
    try:
        ...
        flag = False
        d = run_sql(f"select * from fkspsheet where ifnull(cwjsq,'')='' or cwjsq is null")
        if len(d)>0:
            for r in d:
                htjsq = None
                cwjsq = None
                ...
                s.query(fkspsheet).filter(fkspsheet.rid == str(r.get('rid', ''))).update(update_json)
                flag = True
...
@any_route('/api/saier/payment_approval/product/batch_import/paid', methods=['POST', 'GET'])
@require_token
async def view_saier_payment_approval_product_batch_import_paid(request):
    s = Session()
    user = request.current_user
    j = await request.json()
    try:
        ...
        djws = j.get('djws', '2')
        cwjsqz = j.get('cwjsqz', '')
        qsrq = j.get('qsrq', '2011-01-01')
        jsrq = j.get('jsrq', datetime.now().strftime('%Y-%m-%d'))
        cpbh1 = j.get('cpbh1', '')
        fpje = j.get('fpje', 0)
        ...
        if fklx != '提前付款':
            return json_result(1, '无可导入数据', {'rows': [], 'bgtt': ''})
        ...
        # 1) 找 cywyzd 列表（bgpm）
        # 注意：number/father 需映射为 rid/pid
        if sfkp == '是':
            if djws == '1':
                sql = (
                    "select cywyzd,fphm,gcmc from bgmxdsheet "
                    "where ((gcmc=:gcmc) or ((sccj1id=:sccj1id) and (sccj1id<>''))) "
                    "and (fphm=:fphm) and (sfdb<>'是') and (zzsl>0) and (gdry=:gdry)"
                )
                ds = run_sql(sql, {'gcmc': csmc, 'sccj1id': gcbh, 'fphm': wxfp, 'gdry': user.username})
            else:
                sql = (
                    "select RMBkh,zzsl,cywyzd,yfje,fphm,wxht,cght,gcmc,jhrq,chyrq,scrq,sjcy,"
                    "tsl,zycpbh,gczj,zwpm,gcjg,jldw,bzdw,chxs,chsl,wxjg,mjdj1,wxzj,zkfy1,"
                    "ywry,ywrya,gdry,wxwyzd,jsfs,mjzj,wxbm1,cgbm1,fktt,ysqje,kpgc,zgrqj "
                    "from cymxsheet "
                    "where ((gcmc=:gcmc) or ((sccj1id=:sccj1id) and (sccj1id<>''))) "
                    "and (fpsb1='是') and (zzsl>0) "
                    "and (((chyrq>=:cwjsqz) and (chyrq is not null)) or ((sjcy>=:cwjsqz2) and (sjcy is not null)) "
                    "or ((zgrqj>=:cwjsqz1) and (zgrqj is not null))) "
                    "and (cpbh like :cpbh) and (khmc like :khmc) and (gdry=:gdry)"
                )
                ds = run_sql(sql, {
                    'gcmc': csmc, 'sccj1id': gcbh, 'cwjsqz': cwjsqz, 'cwjsqz1': cwjsqz, 'cwjsqz2': cwjsqz,
                    'cpbh': f"%{cpbh1}%", 'khmc': f"%{khmc}%", 'gdry': user.username
                })
        else:
            if djws == '1':
                sql = (
                    "select RMBkh,ysfp,zzsl,cywyzd,yfje,fphm,wxht,cght,gcmc,jhrq,chyrq,scrq,sjcy,"
                    "tsl,zycpbh,gczj,zwpm,gcjg,jldw,bzdw,chxs,chsl,wxjg,mjdj1,wxzj,zkfy1,"
                    "ywry,ywrya,gdry,wxwyzd,jsfs,mjzj,wxbm1,cgbm1,fktt,ysqje,kpgc,zgrqj "
                    "from cymxsheet "
                    "where ((gcmc=:gcmc) or ((sccj1id=:sccj1id) and (sccj1id<>''))) "
                    "and ((fphm=:fphm) or (ysfp=:ysfp)) and (fpsb1='是') and (zzsl<=0) "
                    "and (gdry=:gdry) "
                    "and (((chyrq>=:cwjsqz) and (chyrq is not null)) or ((sjcy>=:cwjsqz2) and (sjcy is not null)) "
                    "or ((zgrqj>=:cwjsqz1) and (zgrqj is not null)))"
                )
                ds = run_sql(sql, {
                    'gcmc': csmc, 'sccj1id': gcbh, 'fphm': wxfp, 'ysfp': wxfp,
                    'gdry': user.username, 'cwjsqz': cwjsqz, 'cwjsqz1': cwjsqz, 'cwjsqz2': cwjsqz
                })
            else:
                sql = (
                    "select RMBkh,zzsl,cywyzd,yfje,fphm,wxht,cght,gcmc,jhrq,chyrq,scrq,sjcy,"
                    "tsl,zycpbh,gczj,zwpm,gcjg,jldw,bzdw,chxs,chsl,wxjg,mjdj1,wxzj,zkfy1,"
                    "ywry,ywrya,gdry,wxwyzd,jsfs,mjzj,wxbm1,cgbm1,fktt,ysqje,zgrqj "
                    "from cymxsheet "
                    "where ((gcmc=:gcmc) or ((sccj1id=:sccj1id) and (sccj1id<>''))) "
                    "and (fpsb1='是') and (zzsl<=0) "
                    "and (((chyrq>=:cwjsqz) and (chyrq is not null)) or ((sjcy>=:cwjsqz2) and (sjcy is not null)) "
                    "or ((zgrqj>=:cwjsqz1) and (zgrqj is not null))) "
                    "and (((htrq>=:qsrq) and (htrq<=:jsrq)) or (htrq is null)) "
                    "and (cpbh like :cpbh) and (khmc like :khmc) and (gdry=:gdry)"
                )
                ds = run_sql(sql, {
                    'gcmc': csmc, 'sccj1id': gcbh, 'cwjsqz': cwjsqz, 'cwjsqz1': cwjsqz, 'cwjsqz2': cwjsqz,
                    'qsrq': qsrq, 'jsrq': jsrq, 'cpbh': f"%{cpbh1}%", 'khmc': f"%{khmc}%", 'gdry': user.username
                })
        ...
        for r in ds:
            ...
            # 过滤已存在产品资料（前端会重复导入）
            # 这里按 rid/pid 规则，由前端避免重复；后端也可做防重逻辑

            # gchk 条件过滤（是否开票区分）
            if sfkp == '是':
                check = run_sql(
                    "select wstt from gchk "
                    "where (jsrm=:jsrm) and (wxfp=:wxfp) and (sfjq<>'是') "
                    "and (gcmc1=:gcmc1) and (chrq<=:cwjsqz) and (chrq is not null) "
                    "and (wstt=:wstt) and (fpwk<>'是') and (gcmc=:gcmc)",
                    {'gcmc': cwgc, 'gcmc1': r.get('gcmc',''), 'wstt': wstt, 'jsrm': jsrm,
                     'cwjsqz': cwjsqz, 'wxfp': r.get('fphm','')}
                )
            else:
                check = run_sql(
                    "select wstt from gchk "
        ...
        for cywyzd in bgpm:
            d2 = run_sql(
                "select * from cymxsheet where ((gcmc=:gcmc) or ((sccj1id=:sccj1id) and (sccj1id<>''))) "
                "and (fpsb1='是') and (cywyzd=:cywyzd)",
                {'gcmc': csmc, 'sccj1id': gcbh, 'cywyzd': cywyzd}
            )
            for r in d2:
                ssqje1 = round((float(r.get('gczj',0)) - float(r.get('zkfy1',0)) -
                                float(r.get('yfje',0)) - float(r.get('ysqje',0))) * 1000) / 1000

                # 多笔审请金额
                yssje1 = 0
                dsum = run_sql(
                    "select ifnull(sum(seje),0) as seje1 from fkspsheet "
                    "where (cywyzd=:cywyzd) and (pid<>:pid)",
                    {'cywyzd': cywyzd, 'pid': rid}
                )
                if len(dsum) > 0:
                    yssje1 = float(dsum[0].get('seje1', 0))
                ...
                # 到票明细
                dpmx = ''
                dm = run_sql("select fphm from gchksheet where (cywyzd=:cywyzd)", {'cywyzd': cywyzd})
                for x in dm:
                    dpmx = x.get('fphm','') if dpmx == '' else dpmx + "\r\n" + x.get('fphm','')

                # 补报发票（bgmxdsheet）
                bbfph = ''
                sbbfph = ''
                d3 = run_sql("select pid,yfph,cywyzd,yfje,fphm,wxht,cght,gczj,zkfy1,ysfp from bgmxdsheet where (cywyzd=:cywyzd)",
                             {'cywyzd': cywyzd})
                for x in d3:
                    if x.get('yfph','') == r.get('fphm','') and x.get('fphm','') != r.get('fphm',''):
                        if bbfph == '':
                            bbfph = x.get('fphm','')
                            sbbfph = x.get('ysfp','')
                        else:
                            bbfph = bbfph + '/' + x.get('ysfp','')

                # 抬头/客户信息（bgmxd，number→rid 映射）
                fktt = ''
                khmc1 = ''
                rmbkh1 = ''
                if len(d3) > 0:
                    number_pid = d3[0].get('pid', '')
                    if number_pid != '':
                        d4 = run_sql("select fphm,bggs,khmc,RMBkh from bgmxd where (rid=:rid)",
                                     {'rid': number_pid})
                        if len(d4) > 0:
                            fktt = d4[0].get('bggs','')
                            khmc1 = d4[0].get('khmc','')
                            rmbkh1 = d4[0].get('RMBkh','')

                # 发票字母后缀映射
                fphm = r.get('fphm','')
                d5 = run_sql(
                    "select fphm from cymxsheet "
                    "where (cywyzd=:cywyzd) and (fphm in :fphms)",
                    {'cywyzd': cywyzd, 'fphms': tuple([fphm + chr(x) for x in range(65, 84)])}
                )
                if len(d5) > 0:
                    fphm = d5[0].get('fphm','')
                ...
                htyf = 0
                if r.get('cght',''):
                    dx = run_sql("select sfsh from cght where (hthm=:hthm)", {'hthm': r.get('cght','')})
                    if len(dx) > 0:
                        cxsb = dx[0].get('sfsh','')

                    dy = run_sql("select ifnull(sum(seje),0) as seje1 from fkspsheet3 where (hthm=:hthm)",
                                 {'hthm': r.get('cght','')})
                    if len(dy) > 0:
                        htyf = float(dy[0].get('seje1', 0))

                    dz = run_sql(
                        "select ifnull(sum(yfje),0) as yfje1 from fkspsheet2 "
                        "where (hthm=:hthm) and (wxfp=:wxfp) and (cywyzd=:cywyzd)",
                        {'hthm': r.get('cght',''), 'wxfp': r.get('fphm',''), 'cywyzd': cywyzd}
                    )
                    if len(dz) > 0:
                        r['yfje'] = dz[0].get('yfje1', 0)

                # 比值
                bz = 0
                if r.get('RMBkh','') == '是' and float(r.get('mjzj',0)) > 0:
                    bz = round(float(r.get('gczj',0)) / float(r.get('mjzj',0)) * 1000) / 1000
                elif float(r.get('wxzj',0)) > 0:
                    bz = round(float(r.get('gczj',0)) / float(r.get('wxzj',0)) * 1000) / 1000

                row = {
                    'duobishenqing': '是' if yssje1 > 0 else '否',
                    'daopiaomingxi': dpmx,
                    'bbfph': sbbfph if bbfph else '',
                    'bbfywfp': bbfph if bbfph else '',
                    ...
                    'ysqje': r.get('ysqje',0),
                    'yfje': r.get('yfje',0),
                    'lspzje': float(r.get('ysqje',0)) + float(r.get('yfje',0)),
                    'cxsb': cxsb,
                    'htyf': htyf,
                    ...
                    'sjcy': r.get('sjcy',''),
                    'chyrq': r.get('chyrq',''),
                    'cwcy': r.get('zgrqj','') or r.get('chyrq',''),
                    'tsl': r.get('tsl',0),
                    'cpbh': r.get('zycpbh',''),
                    'gczj': r.get('gczj',0),
                    'zwpm': r.get('zwpm',''),
                    ...
                    'gdry': r.get('gdry',''),
                    'htrq': r.get('htrq',''),
                    'fktt': fktt or r.get('fktt',''),
                    'sfhs': '是' if float(r.get('zzsl',0)) > 0 else '否',
                    'gdbm': '',
                    'rmbkh': r.get('RMBkh',''),
                    'wxbm': r.get('wxbm1',''),
                    'cgbm': r.get('cgbm1',''),
                    ...
                    'mjzj': r.get('mjzj',0),
                    'bz': bz,
                    'zkfy': float(r.get('gczj',0)) * float(zkds or 0) / 100,
                    'fklx': fklx,
                    'fkxs': '',  # 前端可继续用主表字段覆盖
                    'ywbm': ywbm,
                    'fpje': fpje,
        ...
        return json_result(1, '查询成功', {'rows': rows, 'bgtt': bgtt})
    except:
        logger.error(trace_error())
        return json_result(-1, trace_error())
    finally:
