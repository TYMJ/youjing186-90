...
async def api_export_uv_finance_list(request):
    ...
    try:
        ...
        hbdmfh = s.execute(text("SELECT hbfh FROM hbdm WHERE hbdm=:hbdm"), {"hbdm": hbdm_raw}).scalar() or ''
        logger.error(hbdm)
        logger.error(hbdmfh)
        logger.error('2222222222222222222222222222222222222')
        ...
        for dept, ws in sheets_data:
            ...
            for row in real_products:
                ...
                if rmbkh:
                    ...
                    logger.error('11111111111111111111111111111111111111111111111111')
                    logger.error(hbdmfh)
                    logger.error(hbdm)
                ...
                ws[f'AM{r_idx}'] = str(row.krcode or '')[:6]
                ...
                ws[f'BB{r_idx}'].number_format = 'YYYY-MM-DD'
                ws[f'BB{r_idx}'].alignment = Alignment(horizontal='left')
            ...
            ws[f'W{r_idx}'] = round(float(cgzj), 2)
            ...
            if bgmxd_info:
                ...
                if supp_items:
                    ...
                    for bg in supp_items:
                        ...
                        ws[f'BB{br_idx}'].number_format = 'YYYY-MM-DD'
            ...
            for rp in real_products:
                ...
                ws.row_dimensions[r_fac].height = 40
...
async def api_export_uv_finance_list_total(request):
    ...
    try:
        ...
        hbdm_raw = str(master.hbdm or '').upper()
        hbdm = 'USD' if hbdm_raw in ('USD', 'USD$') else hbdm_raw
        hbdmfh = s.execute(text("SELECT hbfh as bjdh FROM hbdm WHERE bjdid=:bjdid"), {"bjdid": hbdm}).scalar() or ''
        rmbkh = (master.RMBkh == '是')
    ...
    try:
        ...
        try:
            ...
            while True:
                ...
                # 提取明细字段    对应到582----683
                xsht = str(ws.cell(row=row_idx, column=4).value or '').strip() # D
                ...
                mz = str(ws.cell(row=row_idx, column=15).value or 0)    # O
                jz = str(ws.cell(row=row_idx, column=16).value or 0)    # P
                ...
                hthm = str(ws.cell(row=row_idx, column=50).value or '').strip()   # AX
                zmz = xs * safe_float(mz) if mz else 0
                zjz = xs * safe_float(jz) if jz else 0
                ztj = xs * safe_float(tj) if tj else 0
                ...
                """
                params_detail = {'bjhh': cpbh, 'bjhh2': cpbh1, 'hthm': hthm}
                ...
                logger.error(f"第{row_idx-2}行：采购合同{hthm} 产品编号{cpbh1} 三步连环查明细结果: {row_detail}")
                if row_detail:
                    ...
                    if '赠送' in xsht:
                        ...
                        if xsht != '' and xsht != db_wxht:
                            sql_wx = "SELECT wxwyzd FROM wxhtsheet WHERE order_id=:order_id AND bjhh=:bjhh AND khmc=:khmc"
                            row_wx = s.execute(text(sql_wx), {'order_id': xsht, 'bjhh': cpbh, 'khmc': khmc}).fetchone()
                            if row_wx:
                ...
                sql_sum = """
                    ...
                    WHERE (bjhh=:bjhh OR khhh=:bjhh2) AND hthm=:hthm
                """
                params_sum = {'bjhh': cpbh, 'bjhh2': cpbh1, 'hthm': hthm}
                if wyzd:
                ...
                if '赠送' in hthm:
                    ...
                    # ycydsheet 里查历史出货数量，条件同采购汇总的三步连环，且发票未开且未作废的记录才算历史出货量
                    his_sql = f"""
                        ...
                        WHERE zycpbh=:cpbh 
                          ...
                          AND (chyrq IS NULL )
                    """
                    his_params = {'cpbh': cpbh, 'cght': hthm}
                        # AND fpsb1<>'是' 
                        # AND ywchy<>'是' 
                    if wyzd == '':
                    ...
                    chhj2 = cyhj1 + zsl1
                logger.error(f"第{row_idx-2}行：采购合同{hthm} 产品编号{cpbh1} 历史出货合计：{cyhj1} 当前出货数量：{zsl1} 超发预警线：{chhj2}")
                # --- 最终超发与作废熔断 (卫语句提前返回) ---
                if (cgsl < chhj2) or (xjht == '作废'):
                    if cgsl < chhj2:
                        errors_list.append(f"第{row_idx-2}条记录：产品编号：{cpbh1} 采购合同：{hthm} 出货数量{chhj2}大于采购数量{cgsl}请核实!")
                    if xjht == '作废':
                        errors_list.append(f"第{row_idx-2}条记录：产品编号：{cpbh1} 采购合同：{hthm} 作废!")
                ...
                zsong = '否'
                if '赠送' in xsht and '赠送' in hthm: zsong = '是'
                elif '赠送' in xsht: zsong = '客人'
                elif '赠送' in hthm: zsong = '工厂'
                ...
                item = {
                    ...
                    "fktt": fktt, "wxtj": final_tj, "wxht": xsht, "khht": '',
                ...
                if is_amazon:
                    item.update({
                        ...
                        "cjhjsj": ws.cell(row=row_idx, column=27).value,
            ...
            # ==============================================================
            err_filename = f"未导入详情{fphm}{time.strftime('%Y-%m-%d_%H-%M-%S')}.txt"
            ...
            if mode == '1':
                ...
                logger.error(f"底栏提取: footer_data={footer_data}, i的数量={row_idx}")
            # --- 组装并返回结果 ---
            result_data = {
                "header": header_data,
                "items": items,
...
async def api_export_declaration(request):
    ...
    user= request.current_user
    # user_info=get_user_path(user.username)
    user_info=get_user_path('李辉')
    user_path=user_info.get('path', '')
    user_pos=user_info.get('position', '')
    is_valid_role = False
    if ('外销' in user_path) or ('单证' in user_pos) or ('外销' in user_pos):
        is_valid_role = True
    else:
    ...
    try:
        ...
        ujsb = '1' if s.execute(text("SELECT  xm FROM cyzglsheet WHERE zm='优景报关模板' AND xm=:xm limit 1" ), {'xm': str(row_cymx.kh_id)}).fetchone() else ''
        ...
        row_wfgs = s.execute(text("SELECT  ywmc FROM wfgs WHERE wfgs=:wfgs limit 1"), {'wfgs': gs}).fetchone()
        ...
        if hr1 == '1':
            ...
            if '1' in ed:
                if da1 == '3':
                    ...
                    tpl = 'hbbgd.xlsx'
                logger.error(f"选择的报关单模板: {tpl}")
                if not os.path.exists(os.path.join(tpl_dir, tpl)): return json_result(-1, f"找不到模板: {tpl}")
                file_path=os.path.join(tpl_dir, tpl)
                logger.error(f"模板打印：{os.path.join(tpl_dir, tpl)}")
                wb = load_workbook(file_path)  # 加载模板 

                i1 = len(valid_items) # 有效的 总条数
                divider = 6 if da1 == '3' else (12 if da1 == '1' else 15)  # 根据字段不同  来复制  sheet页面
                i = int(i1 / divider)  # 取整
            ...
            if (not lvpd and uae != '1') and ('2' in ed):
                ...
                for row in valid_items:
                    ...
                    # 1. 只保留对 wypm (外销品名表) 的查询，彻底干掉 tspmb
                    # row_djpmw = s.execute(text("SELECT djpmw FROM wypm WHERE cpbh=:cpbh OR krhh=:cpbh"), {'cpbh': row.cpbh}).fetchone()
                    djpmy_val = str(row.djpmy or '').upper()
                    krcode_val = str(row.krcode or '')
                    # if row_djpmw:
                    #     ws[f'{DD}{21+d}'] = f"{djpmy_val}\nCUSTOMS CODE:{krcode_val}"
                    #     ws[f'{DD}{22+d}'] = str(row_djpmw.djpmw or '')
                    # else:
                        # 没查到 wypm：英文品名、海关码、以及默认的外文品名(row.djpmw) 全部挤在 21 行换行显示
                    default_djpmw = str(row.djpmw or '')
                    ws[f'{DD}{21+d}'] = f"{djpmy_val}\nCUSTOMS CODE:{krcode_val}\n{default_djpmw}"
                        # 如果您的模板第 22 行原本有脏数据，可以加一句 ws[f'{DD}{22+d}'] = '' 来清空它
            ...
            if (not lvpd and uae != '1') and ('3' in ed):
                ...
                for row in valid_items:
                    ...
                    ws[f'{B}{24+d}'] = f"{row.cpbh}    "

                    # row_tspmb = s.execute(text("SELECT djpmy,djyxb FROM tspmb WHERE cpbh=:cpbh AND djpmy<>''"), {'cpbh': row.cpbh}).fetchone()
                    rich_djpmy = str(row.djpmy).upper()
        ...
        # ================= [ 第二大分支: FF / 俄罗斯 (俄英双语模板) ] =================
            ...
            if '2' in ed and (lvpd ==''):    
                ...
                for number, row in enumerate(valid_items, 1):
                    ...
                    # row_djpmw = s.execute(
                    #     text("SELECT djpmw FROM wypm WHERE cpbh=:cpbh OR krhh=:cpbh"), 
                    #     {'cpbh': row.cpbh}
                    # ).fetchone()
                    djpmy_upper = str(row.djpmy or '').upper()
                    # if row_djpmw:
                    #     djpmw_val = str(row_djpmw.djpmw or '')
                    #     ws[f'B{r_idx}'] = f"{djpmy_upper}\n{djpmw_val}"  # ?? 
                    # else:
                
                    default_djpmw = str(row.djpmw or '')
                    ws[f'B{r_idx}'] = f"{djpmy_upper}/{default_djpmw}"

                    # ws[f'C{r_idx}'] = f"{row.czkrhh}    " if row.czkrhh else f"{row.cpbh}    "
                    ws[f'C{r_idx}'] = f"{row.cpbh}    "
            ...
            if '3' in ed:
                ...
                for number, row in enumerate(valid_items, 1):
                    ...
                    # row_djpmw = s.execute(
                    #     text("SELECT djpmw FROM wypm WHERE cpbh=:cpbh OR (krhh=:cpbh AND krhh != '')"), 
                    #     {'cpbh': str(row.cpbh or '')}
                    # ).fetchone()

                    djpmy_upper = str(row.djpmy or '').upper()

                    # if row_djpmw:
                    #     ws[f'B{r_idx}'] = f"{djpmy_upper}\n{str(row_djpmw.djpmw or '')}"
                    # else:
                    ws[f'B{r_idx}'] = f"{djpmy_upper}/{str(row.djpmw or '')}"
        ...
        # ================= [ 第三大分支: 通用模板 ] =================
            ...
            if '3' in ed and (lvpd ==''):    
                ...
                for number, row in enumerate(valid_items, 1):
                    ...
                    ws[f'B{r_idx}'] = f"{row.cpbh}    "
                    row_tspmb = s.execute(text("SELECT djpmy,djyxb FROM tspmb WHERE cpbh=:cpbh AND djpmy<>''"), {'cpbh': row.cpbh}).fetchone()
                    ws[f'D{r_idx}'] = str(row.djpmy).upper()
        ...
        if lvpd == '1' or uae == '1':
            if '2' in ed:
                ...
                for row in items:
                    ...
                    row_zscp = s.execute(
                        text("SELECT krtm, lvkrtm FROM zscp WHERE cpbh=:cpbh OR (krhh=:cpbh AND krhh != '' )"), 
                        {'cpbh': str(row.cpbh or '')}
                    ).fetchone()
                    if row_zscp:
            ...
            if '3' in ed:
                ...
                ogger.error(f"模板打印：{os.path.join(tpl_dir, tpl)}")
                wb = load_workbook(os.path.join(tpl_dir, tpl))
                ws = wb.worksheets[0]
                k, number = 0, 24
                if lvpd == '1':
                ...
                for row in items:
                    ...
                    # 加上了防空值判断，既保持了 Python 的简洁，又恢复了老 Pascal 的安全性
                    row_zscp = s.execute(
                        text("SELECT krtm,lvkrtm,bzcd,bzkd,bzgd,nhzd,nhkd,nhgd FROM zscp WHERE cpbh=:cpbh OR (krhh=:cpbh AND krhh != '')"), 
                        {'cpbh': str(row.cpbh or '')}
                    ).fetchone()
                    if row_zscp:
