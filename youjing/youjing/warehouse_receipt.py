...
#             # contract_total=float(3) #测试使用，正式使用时删除
#             # 验证1：实际箱数必须大于等于出库箱数
...
#         if (result['path'] == '' or result['path'] is None):
...
#             jsql = s.query(inventorydetail.rid).filter(inventorydetail.wyzd == wyzd).first()
#             if jsql:
#                 return json_result(-1, f'第{hh}行产品{item["ItemNo"]}的已经入仓,操作失败！')
