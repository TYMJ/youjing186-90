...
def _collect_cggd_rows(rid_list, user_name):
    ...
    for rid in rid_list:
        rs = run_sql(
            """
            SELECT rid, hthm, gdry, sccj, cgry, cs_id, wxht, ywry
            FROM cggd
            WHERE rid=:rid
            """,
            {'rid': str(rid)}
...
def _fill_detail_sheet(ws, lines, company, cpbh_show, sign_path):
    ...
    for idx, row in enumerate(lines, start=1):
        ...
        ws[f'C{excel_row}'] = row.get('khhh') or row.get('bjhh')
        ...
        if is_jingchi:
            ws[f'L{excel_row}'] = _get_dforder(row.get('wxht'))

    head_cell = 'M1' if is_jingchi else 'A1'
    ...
    if sign_path:
        _try_add_image(ws, sign_path, f'B{sum_row + 2}')
        ...
        for factory in factories:
            ...
            for buyer in buyers:
                ...
                for bjhh, lines in grouped.items():
                    ...
                    _fill_detail_sheet(ws_detail, lines, company, cpbh_show, sign_path)
