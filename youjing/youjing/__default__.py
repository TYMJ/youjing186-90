...
def module_share_new(module, to_users, record_id, user, s, old_users=[]):
    try:
        ...
        if len(old_users)>0 and 'all' in old_users:
            s.query(table).filter(table.record_id==record_id).delete(synchronize_session=False)
        ...
        if len(old_users)>0 and not 'all' in old_users:
...
def get_user_path(username, where_str=''):
    try:
        ...
        if res.get('empty') == False and (res.get('position') == None or res.get('position') == '') and res.get('memo') != '' and res.get('memo') != None:
            res['position'] = res.get('memo')
        if (oid == "" or oid == None) and res.get('empty') == False:
            res['path'] = '我的公司'
            return res
...
def user_task_new(module, rid, key_field, title, content, user, s, to_list=[], delete=False):
    try:
        ...
        logger.error('tttttttttttttttttttttttttttttttttttt')
        logger.error(t)
        logger.error(o)
        d = s.query(o).filter(o.rid == rid).first()
        if not d:
            return {"code": 0, "msg": "无记录无需处理"}
...
def supplier_workflow_end(self, rid, s):
    try:
        ...
        if splx == '认证申请' :
            ...
            if d.ewfz != None and float(d.ewfz)!=0: 
                if m.jspf != None and float(m.jspf) >= 60:
                    ...
                    if float(d.ewfz) + float(m.jspf) >= 60:
                        m.ewfz = 60 - float(m.jspf) if m.jspf != None and float(m.jspf) < 60 and float(m.jspf) > 0 else 0
                        m.hzdj1 = 60
                    else:
                        m.ewfz = float(d.ewfz)
                        m.hzdj1 = float(d.ewfz) + float(m.jspf)
            ...
            ewfz = float(m.ewfz) if m.ewfz != None else 0
            if d.hzdj1 == None and float(d.hzdj1) > 0:
                if float(d.hzdj1) >= 60:
                    m.hzdj1 = float(d.hzdj1)
                    m.ewfz = 0
                elif float(d.hzdj1) > 0 and float(d.hzdj1) < 60 and ewfz + float(d.hzdj1) >= 60:
                    m.hzdj1 = 60
                    if 60 - ewfz > 0:
                        m.ewfz = 60 - ewfz
                    else:
        ...
        hzdj1 = float(d.hzdj1) if d.hzdj1 != None else 0
        ewfz1 = float(m.ewfz) if m.ewfz != None else 0
        hzdj2 = float(m.jspf) if m.jspf != None else 0
        ewfz2 = float(d.ewfz) if d.ewfz != None else 0
        jspf = float(d.hzdj1) - ewfz1
        # flag = 0
        for f in fields:
            if f in SYS_FIELDS:
                continue
            if (f == 'shjg' or f == 'shr') and splx == '认证申请' or f == 'rzry':
                continue
            if f == 'ewfz' and hasattr(d,'ewfz'):
                if splx == '认证申请':
                    if ewfz2 > 0:
                        if hzdj2>=60:
                            setattr(d,'ewfz', 0) # hzdj2 - 60 if hzdj2 - 60 > 0 else 0)
                            setattr(d,'hzdj1', 61 if hzdj2 == 60 else hzdj2)
                        elif hzdj2 < 60 and ewfz2 + hzdj2 >= 60:
                            setattr(d,'ewfz', 60 - hzdj2)
                            setattr(d,'hzdj1', 60)
                        elif hzdj2 < 60 and ewfz2 + hzdj2 < 60:
                            setattr(d,'ewfz', 0)
                            setattr(d,'hzdj1', hzdj2 + ewfz2) 
                    else:
                        # setattr(d,'ewfz',float(m.jspf))
                        setattr(d,'hzdj1',float(m.jspf))
                else:
                    if hzdj1 > 60:
                        setattr(d,'ewfz', 0)
                        # setattr(d,'hzdj1', hzdj1)
                    elif hzdj1 <= 60 and ewfz1 >= 60:
                            setattr(d,'ewfz', 60 - hzdj1)
                            setattr(d,'hzdj1', 60)
                    elif hzdj1 + ewfz1 >= 60 and hzdj1 < 60 and ewfz1 > 0:
                        setattr(d,'ewfz', ewfz1)
                        setattr(d,'hzdj1', 60)
                    elif hzdj1 + ewfz1 < 60:
                        setattr(d,'ewfz',ewfz1)
                        setattr(d,'hzdj1',hzdj1 + ewfz1)
                continue
...
def customer_claim_workflow_end(rid, self, s):
    try:
        m = s.query(khsp).filter(khsp.rid==rid).first()
        if not m:
            return {'code':-1, 'msg':'未找到记录信息，不能进行此操作'}
        m.shjg = '通过'
        m.shsq = self.user.username
        m.pzrq = time.strftime("%Y-%m-%d")
        s.add(m)
        return {'code':1, 'msg':'操作成功'}
    except Exception as e:
        logger.error(trace_error())
...
def quotation_workflow_end(rid, s):
    s = Session()
    try:
        # time.sleep(3)
        m = s.query(bj).filter(bj.rid==rid).first()
        if not m:
            return {'code':-1, 'msg':'未找到记录信息，不能进行此操作'}
        if m.bjql == '不通过':
            # and m.sfms != '是':
            ...
            m.spsq = ''
            m.bjql = '待审批'
            logger.error('报价审批通过，更新状态成功 000- 不通过')
        else:
            m.bjql = '通过'
            logger.error('报价审批通过，更新状态成功 1111- 通过')
        s.add(m)
        s.commit()
        return {'code':1, 'msg':'操作成功'}
    except Exception as e:
...
def shipping_workflow_end(rid, s):
    try:
        # time.sleep(3)
        m = s.query(chyjh).filter(chyjh.rid==rid).first()
        if not m:
            return {'code':-1, 'msg':'未找到记录信息，不能进行此操作'}
        # logger.error('出运计划审批通过，更新状态成功 1111- 通过')
        m.dzqr = '结束'
        s.add(m)
        # logger.error('出运计划审批通过，更新状态成功 2222- 结束')
    ...
    try:
        ...
        if m == '采购合同':
            ...
            if res.get('code') != 1:
                ...
                return json_result(-1, res.get('msg','操作失败'))
        if m == '客户索赔':
            res = customer_claim_workflow_end(rid, self, s)
            if res.get('code') != 1:
                s.rollback()
                logger.error(res.get('msg','操作失败'))
                return json_result(-1, res.get('msg','操作失败'))
        if m == '客户报价':
            # res = quotation_workflow_end(rid, s)
            thread_run(quotation_workflow_end, rid, None)
            # if res.get('code') != 1:
            #     s.rollback()
            #     logger.error(res.get('msg','操作失败'))
            #     return json_result(-1, res.get('msg','操作失败'))     
        if m == '出运计划':
            res = shipping_workflow_end(rid, s)
            if res.get('code') != 1:
                s.rollback()
                logger.error(res.get('msg','操作失败'))
                return json_result(-1, res.get('msg','操作失败'))
...
# 报价审批工作流流转执行代码 20260418 更新
def quotation_workflow_task_flow(rid, status, memo, module, user, s):
    try:
        bj_id = ''
        bjry = ''
        ...
        if d:
            ...
            u = s.query(sys_user).filter(sys_user.username == bjry).first()
            res = module_xxck_new([row], u, s)
            if res.get('code',1) != 1:
                return {'code':-1, 'msg':res.get('msg')}
 
            if status == 2:
                res = module_workflow_cancel(module, rid, status, user, s, memo)
                if res.get('code') != 1:
                    s.rollback()
                    logger.error(res.get('msg','操作失败'))
                    return {'code':-1, 'msg':res.get('msg')}
...
def workflow_module_check(rid, module, s):
    try:
        ...
        if module == '采购报价' or module == '外销合同' or module == '客户报价': 
            ...
            d = s.query(t.cysq,t.khmc).filter(t.rid==rid).first()
            if not d:
                return {'code': -1, 'msg': '未找到记录信息，工作流启动失败'}
            if d.khmc and d.khmc!= None and 'BEST PRICE' in d.khmc.upper():
                return {'code': -1, 'msg': '客户名称包含BEST PRICE的出运计划无需启动工作流'}
            ...
            d = s.query(t.tjsh).filter(t.rid==rid).first()
            if d and (d.tjsh == '' or d.tjsh == None):
                return {'code':-1, 'msg':'审核申请为空，工作流启动失败'}
        elif module == '客户索赔':
            d = s.query(t.pksq).filter(t.rid==rid).first()
            if d and (d.pksq == '' or d.pksq == None):
                return {'code':-1, 'msg':'赔款申请为空，工作流启动失败'}
...
def workflow_flow_out_check(rid, instance_id, task_id, s):
    try:
        ...
        if module == '出运计划':
            ...
            if unit == '审批处理': 
                if d.splx == '认证申请' and  (d.rzry == '' or d.rzry == None):
                    return {'code': -1, 'msg': '请先填写认证人员'}
                if d.splx == '加分申请' and  (d.shjg == '' or d.shjg == None or d.ewfz == None or d.ewfz == 0):
                    return {'code': -1, 'msg': '请先填写审核结果和额外分值'}    
        elif module == '采购计划':
            if unit == '合同审批' and (d.nbht == None and d.ywht == None and d.stht == None and d.nbht == '' and d.ywht == '' and d.stht == ''):
                return {'code': -1, 'msg': '宁波合同、义乌合同、汕头合同不能都为空，工作流流转失败'}
        return {'code':1, 'msg':'操作成功'}
    except Exception as e:
        logger.error(trace_error())
        return {'code':-1, 'msg':trace_error()}
...
def workflow_cancel(self):
    if self == None:
        #升级上来的数据是空的
        return
    logger.error(self.module)
    logger.error(self.record_id)
    logger.error(self.user)
    logger.error(self.current_unit)

evts.on(WORKFLOW_CANCEL,workflow_cancel)


def module_workflow_cancel(module, rid, status, user, self=None, memo=''):
    if self is None:
        s = Session()
    else:
        s = self
    try:
        ...
        fields = [
            ...
            {'field':'ywht', 'value':''},
            {'field':'stht', 'value':''},
            {'field':'bjql', 'value':'待审批'},
            {'field':'dzqr', 'value':'否'},
        ]
        if not d:
            return {'code': -1, 'msg': '未找到记录信息'}
        if user.rid == d.uid and  (d.wf_status == 2 or (hasattr(d, 'bjql') and d.bjql == '通过') or (hasattr(d,'sfhz') and d.sfhz == '通过') or (hasattr(d,'jhqr') and d.jhqr == '通过')):
            return {'code': -1, 'msg': '该记录已通过审批，不能进行此操作'}
        # if d.uid == user.rid and module != '出运计划' and module != '采购计划' and (d.wf_status == 1 or (hasattr(d, 'bjql') and d.bjql == '待审批') or (hasattr(d,'sfhz') and d.sfhz == '待审批') or (hasattr(d,'jhqr') and d.jhqr == '待审批')):
        #     return {'code': -1, 'msg': '只有业务人员才能进行此操作'}
        for f in fields:
        ...
        if status == 2:
            ...
            if hasattr(d, 'wpyy'):
                d.wpyy = memo
            d.wf_unit = ''
            f = s.query(sys_workflow_instance).filter(sys_workflow_instance.module==module, sys_workflow_instance.record_id==rid, sys_workflow_instance.status==0).order_by(sys_workflow_instance.sid.desc()).first()
            if f:
                f.status = 2
                f.cancel_uid = user.rid
                f.cancel_time = time.strftime("%Y-%m-%d %H:%M:%S")
                f.op_time = time.strftime("%Y-%m-%d %H:%M:%S")
                s.add(f)
                s.query(sys_workflow_task).filter(
                    sys_workflow_task.instance == f.rid,
                    sys_workflow_task.module == module,
                    sys_workflow_task.status == 0
                ).update({'status': 3, 'end_time': time.strftime("%Y-%m-%d %H:%M:%S")}, synchronize_session=False)
                s.query(sys_workflow_user_task).filter(
                    sys_workflow_user_task.instance == f.rid,
                    sys_workflow_user_task.module == module,
                    sys_workflow_user_task.status == 0
                ).update({'status': 3, 'op_time': time.strftime("%Y-%m-%d %H:%M:%S")}, synchronize_session=False)
...
async def api_saier_workflow_cance_after(request):
    ...
    try:
        module = j.get('module')
        rid = j.get('rid')
        status = j.get('status', 2)
        res = module_workflow_cancel(module, rid, status, user, self=None, memo = '')
        if res.get('code') != 1:
...
#         if user.rid == d.uid and  (d.wf_status == 2):
#             # or (hasattr(d, 'bjql') and d.bjql == '通过') or (hasattr(d,'sfhz') and d.sfhz == '通过') or (hasattr(d,'jhqr') and d.jhqr == '通过')):
#             return json_result(-1, '该记录已通过审批，不能进行此操作')
#         # if d.uid == user.rid and module != '出运计划' and module != '采购计划' and (d.wf_status == 2):
#             # or (hasattr(d, 'bjql') and d.bjql == '待审批') or (hasattr(d,'sfhz') and d.sfhz == '待审批') or (hasattr(d,'jhqr') and d.jhqr == '待审批')):
...
@any_route('/api/module/right/panel/image/view', methods=['POST'])
@require_token
async def view_api_module_right_panel_image_view(request):
    j = await request.json()
    cpbh = j.get('bjhh',None)
    if is_none(cpbh):
        return json_result(ERR_PARAM_NOT_ENOUGH)
    s = Session()
    try:
        if cpbh!=None and cpbh!='':
            row = s.query(zscp.yytp.label('Photo')).filter_by(cpbh = cpbh).first()
            if not row:
                row = s.query(cjcp.yytp.label('Photo')).filter_by(cpbh = cpbh).first()
            if row:
                return json_result(data=row.Photo)
        else:
            return json_result(-1)
    except Exception as e:
       logger.error(trace_error())
       return json_result(ERR_OPERATE)
    finally:
        s.close()