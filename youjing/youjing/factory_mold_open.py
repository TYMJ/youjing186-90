...
async def view_saier_factory_mold_open_cpbh_change(request):
    ...
    try:
        ...
        if rid == '':
            d = run_sql(f"select cpbh from gckm where (kmbh='{str(kmbh)}')  order by rid desc")
        else:
    ...
    try:
        ...
        if tg != '待审批':
            ...
            if tg in ['确认', '通过']:
                ...
                if cdje > 0:
                    ...
                    res = user_task_new('工厂开模', rid, '开模编号','[开模编号]工厂开模需审批', subject[:99], user, s, ['侯柳红'])
                    if res.get('code') != 1:
                        s.rollback()
                        return json_result(res.get('code'), res.get('msg'))
        ...
        if sh == '' and username != '侯柳红':
            if sh == '' and tg == '待审批':
                if sh != '' and sh != username:
                    ...
                    if not d:
                        res = user_task_new('工厂开模', rid, '开模编号','[开模编号]工厂开模需审批', user.username + '产品编号:' + str(cpbh) + '开模需审核,日期:' + sqrq, user, s, [sh])
                        if res.get('code') != 1:
                            s.rollback()
                            return json_result(res.get('code'), res.get('msg'))
        ...
        if spr == username and zjlsp == '通过' and wcqk == '已完成' and hzsb != '' :
            ...
            if sfhz != '通过':
                ...
                res = user_task_new('工厂开模', rid, '开模编号','[开模编号]工厂开模需审批',xxnr, user, s, [sqr])
                if res.get('code') != 1:
                    s.rollback()
                    return json_result(res.get('code'), res.get('msg'))
            else:
...
async def view_saier_factory_mold_open_button_gqdsq(request):
    ...
    try:
        ...
        res = user_task_new('工厂开模', rid, '开模编号','[开模编号]工厂开模需审批',str(xxnr[:99]), user, s, [sh])
        if res.get('code') != 1:
            s.rollback()
            return json_result(res.get('code'), res.get('msg'))
...
async def view_saier_factory_mold_open_button_fksq(request):
    ...
    try:
        ...
        res = user_task_new('工厂开模', rid, '开模编号','[开模编号]工厂开模需审批',str(xxnr[:99]), user, s, [kmsp])
        if res.get('code') != 1:
            s.rollback()
            return json_result(res.get('code'), res.get('msg'))
