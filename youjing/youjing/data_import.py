...
async def _import_photo_from_excel(fn,user,group,kfield,pfield,request,option='update',heard=1):
    try:
        ...
        logger.error(d)
        await messages.message(user.username,{'total':99,'progress':99,'kind':'2','group':group, 'kfield':kfield,'pfield':pfield,'data':d,'option':option},MSG_KIND_NORMAL,request) 
    except:
