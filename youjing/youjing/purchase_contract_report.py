        ...
        if str(row_cght.sfhz) == '通过':
            ...
            if (htjez > cxje1) or (cxjezz > cxjez1):    # 对应源代码 大约 554   只要这张合同的金额，或者今年累计跟这家工厂合作的金额，超过了设定的警戒线
                ...
                if not row_cxgc:  #系统里从来没有这家工厂的诚信档案
                    now_str, date_str = dt.now().strftime('%Y-%m-%d %H:%M:%S') or None, dt.now().strftime('%Y-%m-%d') or None
                    cxgc_rid = get_uuid() # 如果您有封装好的 get_uuid()，可以直接改为 jc_rid = get_uuid()
                    user_rid = user.rid # 获取当前用户的 rid

                    s.execute(sql_text("INSERT INTO cxgc (rid, uid, ctime, gcmc, ywry, ywz, yrrq, sfsh, yrgs, qrrq, chgc, fkry, xgry, cs_id) \
                        VALUES (:rid, :uid, :ctime, :gcmc, :ywry, :ywz, :yrrq, '否', '宁波优景进出口有限公司', null, :chgc, '', '', :cs_id)"), \
                        {'rid': cxgc_rid, 'uid': user_rid, 'ctime': now_str, 'gcmc': zygcmc2, 'ywry': gdry3, 'ywz': gdbm3, 'yrrq': date_str, 'chgc': zygcmc2, 'cs_id': zygcid2})
                    ...
                    if D3 is not None:
                        ...
                        if days_diff > 365:
                            s.execute(sql_text("UPDATE cxgc SET yrrq=:yrrq, ywry=:ywry, ywz=:ywz, sfsh='否', yrgs='宁波优景进出口有限公司', qrrq=null, chgc=:chgc, cs_id=:cs_id WHERE gcmc=:gcmc"), {'gcmc': zygcmc2, 'ywry': gdry3, 'ywz': gdbm3, 'yrrq': dt.now().strftime('%Y-%m-%d'), 'chgc': zygcmc2, 'cs_id': zygcid2})
                            s.execute(sql_text("UPDATE zycs SET cxhc='否' WHERE cs_id=:cs_id"), {'cs_id': zygcid2})
                            tmpstr2.append(f"工厂名称:{zygcmc2}需提交诚信报告")
                            sfsh, cxsb = '待提供', '1'
                        elif days_diff > 330:
            ...
            # ================= [ Part 15~16: 多条明细时 (Outermost 4 else) ] =================
                ...
                try:
                    ...
                    if is_yiwu:
                        ...
                        # 宁波封面
                        if not is_fl:
                            cover_cells.update({'B6': gstt, 'B7': dz, 'B8': lxfs, 'C11': sccj, 'N10': hthm, 'I10': szdh, 'L11': cslxr, 'C12': phone, 'L12': fax, 'D10': htrq[:10], 'K16': 0, 'L16': 0, 'O16': 0, 'F17': result_cn, 'P17': zje1})
...
async def api_saier_ecommerce_inquiry_export(request):
    ...
    try:
        ...
        # --- 核心改造：从创建新文件改为加载模板 ---
        tpl_path = os.path.join(config.data_upload_path, 'template', '电商询价表.xlsx')
        if not os.path.exists(tpl_path):
                return json_result(-1, "未找到模板文件：电商询价表.xlsx")
        ...
        # 针对包含多行文本的单元格，开启自动换行
        for col in ['C', 'G', 'H']:
            # 保留自动换行样式，但不再手动设置列宽，由模板决定
            ws[f'{col}{row_num}'].alignment = Alignment(wrap_text=True, vertical='center', horizontal='center')

        # 6. 生成并保存临时文件
        s_path = config.tmp_path
      
        filename = f"{record_id}_电商询价表.xlsx"
        wb.save(os.path.join(s_path, filename))
...
async def api_saier_shipping_lv_export(request):
    ...
    try:
        ...
        # 4. 加载 LV出运清单 模板 (适配原系统逻辑)
        tpl_path = os.path.join(config.data_upload_path, 'template', 'LV出运清单.xlsx')
        if not os.path.exists(tpl_path):
            return json_result(-1, "未找到报表模板：LV出运清单.xlsx")
        ...
        # 5. 循环填充数据 (从第 3 行模板行开始动态往下插入)
        template_row = 3
        for k, d in enumerate(details, 1):
            row_num = template_row + k
        ...
        # 清理模板行：还原 Pascal 中的 msexcelworksheet.Rows[inttostr(number)].Delete;
        ws.delete_rows(template_row)

        # 6. 生成并返回文件
        s_path = config.tmp_path
        
        filename=str(record_id)+'lv出运清单.xlsx'
        wb.save(os.path.join(s_path, filename))
