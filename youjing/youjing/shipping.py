...
async def view_saier_shipping_cysq_change(request):
    ...
    try:
        ...
        d = run_sql(f"select rid from spwt where dlr='{spsq}' and ('{path2}' like concat('%',replace(cydl,'\\\\','_'),'%') or '{path2}' like concat('%',replace(dxdl,'\\\\','_'),'%')) limit 1")
        if len(d) == 0:
            return json_result(-1,'此人没有审批权限,请重新选择')
        cyhz = ''
        fkqr = ''
        zjhz = ''
        if dzry!='' and dzry!=None and spsq == user.username:
            cyhz = '通过'
            fkqr = '待审批'
        else:
...
async def view_saier_shipping_save_before(request):
    ...
    try:
        ...
        if not d:
            ...
            ysdz = d.sdsm
            ysqr = d.cysq
        if ysqr == '' or ysqr == None:
            data['cdsb'] = 1
        mdg = j.get('mdka', '')
        d = run_sql(f"select ssgj from mdka where mdka=:mdka", {'mdka': mdg})
        if len(d) > 0:
            ssgj = d[0].get('ssgj', '')
            if ssgj != '' and ssgj != None:
                c = run_sql(f"select zwmc,dqzw from mygb where mygb='{ssgj}' limit 1")
                if len(c) > 0:
                    data['zwmc'] = c[0].get('zwmc', '')
                    data['dqzw'] = c[0].get('dqzw', '')
        ...
        for r in lines:
            ...
            bank = r.get('bank', '')
            zh = r.get('zh', '')
            data['lines'][r.get('rid')] = '否'
            xgsb = ''
            if sccj != '' and sccj != None and zzsl == 0 and (bgpm == '' or bgpm == '无' or bgpm == None) and (skhm == '' or bank == '' or zh == '' or skhm == None or bank == None or zh == None) and sccj not in data.get('bank'):
                d = run_sql(f"select fkhm,bank1,zh1 from zycs where cs_id='{sccj}' limit 1")
                if len(d) > 0:
                    data['bank'][sccj] = {'fkhm': d[0].get('fkhm', ''), 'bank1': d[0].get('bank1', ''), 'zh1': d[0].get('zh1', '')}
            if dzzt == '再审':
                c = s.query(chyjh).filter(chyjhsheet.rid == rid).first()
                if c:
                    if (r.get('yjcq') != c.yjcq) or (r.get('jhrq') != c.jhrq) or (r.get('wxrl') != c.wxrl) :
                        xgsb = '1'
                    if r.get('chxs2') != c.chxs2 or (r.get('chsl') != c.chsl) or (r.get('gczj') != c.gczj) :
                        xgsb = '1'
                    if (r.get('wxzj') != c.wxzj) or (r.get('wxmz') != c.wxmz) or (r.get('wxjz') != c.wxjz) :
                        xgsb = '1'
                    if (r.get('bzcd') != c.bzcd) or (r.get('bzkd') != c.bzkd) or (r.get('bzgd') != c.bzgd) :
                        xgsb = '1'
                    if (r.get('wxtj') != c.wxtj) or (r.get('ztj') != c.ztj) or (r.get('qydd') != c.qydd) :
                        xgsb = '1'
                    if r.get('zs') != c.zs or (r.get('jhdd') != c.jhdd) or (r.get('nhrl') != c.nhrl) or r.get('nhwx') != c.nhwx :
                        xgsb = '1'
                    if (r.get('RMBkh') != c.RMBkh or (r.get('yhry') != c.yhry) or (r.get('zkfy') != c.zkfy) or r.get('sfpx') != c.sfpx) :
                        xgsb = '1'
                    if r.get('sccj1') != c.sccj1 and (r.get('sfsj') != c.sfsj) or (r.get('sdjy') != c.sdjy) or (r.get('sfdb') != c.sfdb) or (r.get('zwpm') != c.zwpm) :
                        xgsb = '1'
                    if r.get('ewchy') != c.ewchy and (r.get('dkds') != c.dkds) :
                        xgsb = '1'
                    if xgsb != '' :
                        data['lines'][r.get('rid')] = '是'
                        s.query(cymxsheet).filter(cymxsheet.cywyzd12 == r.get('cywyzd'), func.ifnull(cymxsheet.cywyzd12) != '').update({'sfgd': '再审'}, synchronize_session=False)
...
async def view_saier_shipping_load_check(request):
    ...
    try:
        ...
        if 'AMAZON' in khmc.upper():
            field = 'dxdl'
        path = org.get('path', '').replace('\\', '_') + '_'
        dl_list = []
        c = run_sql(f"select dlr,yhms from spwt where '{path}' like concat('%',replace({field},'\\\\','_'),'%') and ifnull({field},'')<>'' and dlr<>'{user.username}' order by yhms desc")
        for r in c:
            if not r.get('dlr', '') in dl_list:
                dl_list.append(r.get('dlr', ''))
        data['dl_list'] = dl_list
