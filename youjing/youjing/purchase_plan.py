...
async def view_saier_purchase_plan_wxht_change(request):
    ...
    try:
        ...
        for r in d:
            ...
            s.query(wxhtsheet).filter(wxhtsheet.wxwyzd==wxwyzd).update({'sl5': sl6 - sl5, 'sl4': sl4})
...
async def view_saier_purchase_plan_load_check(request):
    ...
    try:
        ...
        cg_org = get_user_path(user.username," and path like '%采购%' ")
        position = cg_org.get('position','')
        empty = cg_org.get('empty',True)
        ...
        spsq_list = []
        d = run_sql(f"SELECT dlr FROM spwt WHERE ('{path1}' LIKE CONCAT('%', replace(wxdl,'\\\\','_'), '%')) AND (spwt.dlr <> '{user.username}') ORDER BY yhms DESC")
        spsq_list = [r.get('dlr','') for r in d if r.get('dlr','') not in spsq_list and r.get('dlr','')!='' and r.get('dlr','')!=None]
     
        return json_result(1, '操作成功', {'zjl': zjl, 'path': path, 'position': position, 'ywry': ywry, 'empty': empty, 'spsq_list': spsq_list})
    except:
        logger.error(trace_error())
        return json_result(-1, trace_error())
    finally:
    ...
    try:
        ...
        for r in items:
            ...
            if r.get('gcpf', 0) <=0:
                ...
                m.gcpf = r.get('gcpf', 0)  or 0
            m.qtcz = r.get('cz', 0)  or 0
            m.ysdj = r.get('cgdj', 0)  or 0
            m.sfqr = '是'
            m.drbz = '否'
            m.sd = '否'
            m.cgsl = r.get('yxdsl', 0)  or 0
            m.cgxs = r.get('xdxs', 0)  or 0
            m.wxrl = r.get('wxrl', 0)  or 0
            m.wlks = r.get('yxdsl', 0)  or 0
            m.sysl = r.get('yxdsl', 0)  or 0
            m.syxs = r.get('xdxs', 0)  or 0
            m.cgjhid = r.get('jhwy', '')
            m.syje = m.zje  or 0
            m.yjcq = r.get('krjq')
            m.chrq = r.get('krjq')
            ...
            if c:
                ...
                # s.add(c)
                s.delete(c)
            htje = htje + (r.get('cgzje')  or 0)
            cgzje = cgzje + (r.get('cgzje1')  or 0)
            htzsl = htzsl + (r.get('yxdsl')  or 0)
            htzxs = htzxs + (r.get('xdxs')  or 0)
            htzmz = htzmz + (r.get('zmz')  or 0)
            htzjz = htzjz + (r.get('zjz')  or 0)
            htztj = htztj + (r.get('ztj')  or 0)
            yhje = yhje + (r.get('yhje')  or 0)
            zkfy = zkfy + (r.get('zkfy')  or 0)
            sysl = sysl + (r.get('yxdsl') or 0)
            syje = syje + (r.get('syje')  or 0)
...
def purchase_process_child_new(pid, cg_rid, user, s):
    try:
        ...
        for r in items:
            ...
            m.zxl = r.get('wxrl', '')
            m.nxrl = r.get('nhrl', 0)
            m.htzxs = r.get('cgxs', 0)
            m.htzsl = r.get('cgsl', 0)
            m.wxcc = r.get('tj', 0)
            m.htzjz = r.get('zjz', 0)
            m.htzmz = r.get('zmz', 0)
            m.htztj = r.get('ztj', 0)
    ...
    try:
        ...
        for r in rids:
            ...
            if d:
                ...
                if len(item_data)==0:
                    return json_result(-1, '没有符合条件的产品记录生成采购合同，可能是低成本确认为否，操作被取消')
