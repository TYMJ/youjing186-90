"""
商检清单 - INV pack sales 导出（对照 Delphi ``INV_pack_sales.pas``）。

模版目录：``{data_upload_path}/template/``（仅 ``.xlsx``，不使用 ``.xls``）
- ``hr=1`` → ``bestorebips.xlsx``
- ``hr=2``（默认，源码 ``hr := '2'``）→ ``bestorebips1.xlsx``

数据：主表 ``sjqd`` + 子表 ``sjqdhbcp``（``pid`` = 主表 ``rid``）。
每条合并产品行输出一份四联单：报关单 / 发票 / 装箱单 / 合同。
文件名：``{fphm};{序号}{zwpm}BIPS.xlsx``

RMB 客户（``RMBkh='是'``）：与源码一致，不拦截；``_resolve_hbdm`` 置 ``HBDM=RMB``，Sheet1 金额用 ``￥#,##0.000``。

Sheet1 表头（本模块按 ``INV_pack_sales.pas``，非 ``source.pas``）：
- ``B12``：合并产品 ``chxs``
- ``A12``：合并产品 ``bzzl``

前端：商检清单详情页预置「扩展」菜单，仅传主表 ``rid``、``hr`` 默认 ``2``。
接口：``POST /api/saier/commodity_inspection/sjqd_inv_sales/export``
"""
...
TEMPLATE_BIPS = "bestorebips"
TEMPLATE_BIPS1 = "bestorebips1"
_MONTH_EN = {
    "01": "JAN",
    "02": "FEB",
    "03": "MAR",
...
def _wrapped_lines(text, width_chars):
    ...
    for para in s.split("\n"):
        if not para:
            total += 1
            continue
        wrapped = textwrap.wrap(para, width=max(1, int(width_chars)))
...
def _template_stem(hr):
    """hr=1 → bestorebips；hr=2 → bestorebips1（与 INV_pack_sales.pas 一致）。"""
...
def _fill_sheet1(ws, header, product, gs, gs1, hbdm, hbdmfh, bgdmzhj, bgdjzhj):
    """报关单 Sheet1 — 对照 INV_pack_sales.pas（B12/A12 取自合并产品行）。"""
    _add_picture_at(
        ws, _col_row_to_coord(4, 24), _image_path_from_tpzx(f"{gs}报关专用章"), x_pad=15, y_pad=8, width=130, height=90
    )
    ...
    # INV_pack_sales.pas: tmpcom5.chxs / tmpcom5.bzzl
    ws["B12"].value = _to_float(product.get("chxs"))
    ws["C12"].value = bgdmzhj
    ws["D12"].value = bgdjzhj
    safe_write(ws, "A12", _upper(get_field_value(product, "bzzl")))

    adjust_row_height(ws, 4)
    adjust_row_height(ws, 8)
    ...
    adjust_row_height(ws, 15)
    adjust_row_height(ws, 16)
    ...
    ws[f"H{row}"].value = "中国"
    adjust_row_height(ws, row, col="A")
...
def _fill_sheet2(ws, header, product, gs, gsyw, hbdm, qsy, qsr, qsn):
    _add_picture_at(
        ws, _col_row_to_coord(3, 48), _image_path_from_tpzx(f"{gs}蓝章名"), x_pad=15, y_pad=8, width=130, height=90
    )
    ...
    adjust_row_height(ws, row, col="B")
...
def _fill_sheet4(ws, header, product, gs, gsyw, hbdm, qsy, qsr, qsn):
    ...
    _add_picture_at(
        ws, _col_row_to_coord(6, 42), _image_path_from_tpzx(f"{gs}蓝章名"), x_pad=8, y_pad=4, width=130, height=90
    )

    safe_write(ws, "C6", _consignee_c6_sheet4(header))
    ws["K6"].value = ""
...
@any_route("/api/saier/commodity_inspection/sjqd_inv_sales/export", methods=["POST", "GET"])
@require_token
async def view_saier_commodity_inspection_sjqd_inv_sales_export(request):
    """
    商检清单详情页 - INV pack sales 导出。
    ...
    参数说明：
    - ``rid``：商检清单主表 rid（详情页必填）
    - ``hr``：``1`` → bestorebips.xlsx，``2`` → bestorebips1.xlsx（默认，同源码）
    - ``mode`` / ``rids``：可选；``mode=2`` 且传 ``rids`` 时批量导出多条主表
    """
    ...
    try:
        ...
        if not os.path.isfile(tpl_path):
            return json_result(-1, f"模版文件缺失：{tpl_stem}.xlsx")
        ...
        zip_name = time.strftime("%Y%m%d%H%M%S") + "_sjqd_inv_sales.zip"
