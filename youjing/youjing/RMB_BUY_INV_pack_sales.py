...
#     """
    ...
    try:
        ...
        if main_info.get('RMBkh', '是') == '否':
            return {'code': -1, 'msg': '此客人非RMB客人，请重新选择格式', 'data': ''}
        ...
        for product in merged_products:
            ...
            bgbgzje += float(main_info.get('mjzj', '0'))
...
#         # --- 2. 生成 Excel (在内存中) ---
        ...
        # 填充报关单项信息
        start_row = 18
        ws.delete_rows(18, amount=10)  # 在A列前插入一列，原A列变为B列
        for idx, form in enumerate(customs_forms):
            ...
            # 否则使用"英文品名"字段值
            # 数量信息
            # safe_write(ws, f'C{row_num}', form.get('hgjldw', ''))
        ...
        for idx, product in enumerate(merged_products):
            ...
            safe_write(ws, f'F{row_num}', f"{product.get('mjdj1', 0):.3f}")
            safe_write(ws, f'G{row_num}', hbdm)
            safe_write(ws, f'H{row_num}', product.get('mjzj', 0))
...
#       
        ...
        for idx, product in enumerate(merged_products):
            ...
            safe_write(ws, f'G{row_num}', hbdm)
            safe_write(ws, f'H{row_num}', f"{product.get('mjdj1', 0):.3f}")
            safe_write(ws, f'I{row_num}', hbdm)
            safe_write(ws, f'J{row_num}', product.get('mjzj', 0))
        if row_num<= 29:
...
# --- API路由 ---
@any_route('/api/saier/export/detailed/INVpacksales/list4', methods=['POST'])
@require_token
