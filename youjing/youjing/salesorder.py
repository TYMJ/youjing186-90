...
async def view_saier_salesorder_hthm_change(request):
    ...
    try:
        ...
        if '退回合同' not in order_id: 
            ...
            #     logger.error('-aaa-')
            l = s.query(wxht.rid,wxht.htwy).filter(wxht.htwy==htwy, wxht.rid!=rid).first()
            if not l:
...
async def view_saier_salesorder_load_check(request):
    ...
    try:
        ...
        org = get_user_path(ywry)
        path = org.get('path','')
        path3 = path.replace('\\','_') + '_'
        logger.error(f"path3--aaa:{path3}")
        spsq_list = []
        d = run_sql(f"SELECT dlr FROM spwt WHERE ('{path3}' LIKE CONCAT('%', replace(wxdl,'\\\\','_'), '%')) AND (spwt.dlr <> '{user.username}') ORDER BY yhms DESC")
        for r in d :
            if r.get('dlr','') not in spsq_list and r.get('dlr','')!='' and r.get('dlr','')!=None:
                spsq_list.append(r.get('dlr',''))
        path1 = 0
        path2 = 0
        if xshl==0:
        ...
        return json_result(code, msg, {'htzd':htzd,'hl':hl,'path1':path1, 'path2':path2, 'ywbm':ywbm, 'path':path, 'spsq_list':spsq_list})
    except:
        logger.error(trace_error())
        return json_result(-1, trace_error())
    finally:
...
async def view_saier_salesorder_purchase_plan(request):
    ...
    try:
        ...
        new_rids = []
        if len(rids)==0:
            return json_result(-1, '请选择要生成采购计划的订单记录')
        for rid in rids:
            d = s.query(wxhtsheet.bjhh,wxhtsheet.bjyw,wxhtsheet.bjpath,wxht.kh_id,wxhtsheet.pid,wxhtsheet.htsl,wxhtsheet.wxwyzd,wxhtsheet.rid,wxht.order_id
            ...
            for r in d:
                if r.bjyw == '' or r.bjyw == None:
                    return {'code': -1, 'msg': '存在业务人员为空的产品记录信息，无法生成采购计划'}
                bjhh = str(r.bjhh)
                group_str = ''
                bjhh = str(r.bjhh)
                bjyw = str(r.bjyw)
                ...
                if kind=='按产品生成':
                    ...
                    if r.bjpath!='' and r.bjpath!=None:
                        path1 = r.bjpath
                    else:
                        if str(r.bjyw) not in user_json:
                            org = get_user_path(str(r.bjyw))
                            user_json[str(r.bjyw)] = org.get('path','').replace('\\','_')
                            path1 = org.get('path','').replace('\\','_')
                        else:
                            path1 = user_json.get(str(r.bjyw))

                    if path1 != "" and path.replace('\\','_') in path1 or bjyw == user.username:
                        group_str = bjyw
                    else:
                        group_str = str(bjhh)+ '-' + bjyw + '-' + kh_id

                if group_str == '':
                    return {'code': -1, 'msg': '产品分组信息存在为空的记录,无法生成采购计划'}
                
                if rid not in new_rids:
                    new_rids.append(rid)
                if group_str not in group:
                    group[group_str] = {'bjhh':str(bjhh), 'bjyw':bjyw, 'kh_id':kh_id, 'order_id': str(r.order_id), 'rid': [str(r.rid)]}
                else:
                    group[group_str]['rid'].append(str(r.rid))
        
        logger.error('----group aaaa-------')
        logger.error(group)
        # if kind!='按产品生成':
        ...
        for k,v in group.items():
            ...
            for l in new_rids:
                ...
                # logger.error('---vvvvvvvv---')
                # logger.error(v)
                rid = get_uuid()
                # if k == '所有':
                    # b = run_sql(f"select * from wxhtsheet where (pid='{l}') and (replace(bjpath,'\\\\','_') like '%%{path}%%') and kh_id='{v['kh_id']}'")
                b = s.query(wxhtsheet).filter(wxhtsheet.rid.in_(v['rid']), wxhtsheet.pid == l).all()
                b = alchemy_object_list_to_dict(b)
                # else:  
                ...
                for  c in b:
                    ...
                    if index == 1:
                        ...
                        order_id = v['bjhh'][:238]
                        kh_id = str(d.get('kh_id',''))
                        logger.error('kk----kkkk')
                        logger.error(k)
                        if kind != '按产品生成' and k == user.username:
                            order_id1 = v['order_id'] + '等;' + time.strftime('%Y%m%d%H%M%S')
                            order_id = v['order_id'] + '等'
                        m.ctime = time.strftime('%Y-%m-%d %H:%M:%S')
                        m.uid = uid
                        m.rid = rid
                        m.order_id1 = order_id1
                        m.order_id = order_id
                        # m.dforder_id = '' #老系统是赋空值的，新系统先注销掉，后续如果有需要再赋值
                    ...
                    if u:
                        ...
                        n.zdml = u.zdml
                        n.sjry = str(u.wbcz)
                    u = s.query(zycs.hzdj1).filter(zycs.cs_id==c.get('sccj1id')).first()
                    if u:
                        n.gcpf = float(u.hzdj1)
                    else:
                        n.gcpf = 0
                    n.wxwyzd = c.get('wxwyzd','')
                    n.pid = rid
                    n.jhrq = c.get('jhrq')
                    n.htrq = time.strftime('%Y-%m-%d')
                    n.rid = get_uuid()
                    n.wyzd = n.rid
                    n.jhwy = m.rid
                    n.krjq = d.get('yjcq')
...
#外销合同的记录复制后
@any_route('/api/saier/salesorder/after/copy', methods=['POST', 'GET'])
@require_token
async def view_saier_salesorder_after_copy(request):
    s = Session()
    user = request.current_user
    # j = await request.json()
    try:
        ...
        org = get_user_path(user.username)
        path = org.get('path','')
        return json_result(1, '操作成功', {'ywbm': ywbm, 'path': path})
    except:
        logger.error(trace_error())
        return json_result(-1, trace_error())
    finally:
