...
#         try:
#             omain = JSONRes.get("omain", "")
            
#             zyh1 = JSONRes.get('zyh1','')
#             zyh =  omain.get("zyh", "")
#             ywy = omain.get("Salesman", "")
#             name = user.username
#             cgy = omain.get("PurchasingAgent", "")
#             ItemsData = omain.get("ItemsData", "")
#             rkrq = omain.get("StorageDate", "")
#             rksj = omain.get("StorageTime", "")
#             jcrq = omain.get("jcrq", "")
#             sdry = omain.get("Operator", "")
#             wxht = omain.get("SalesOrderNo", "")
#             gsmc = omain.get("Exporter", "")
         
#             cght = omain.get("PurchaseOrderNo", "")
#             csmc = omain.get("SupplierShortName", "")
   
#             lhy = omain.get("Collator", "")
#             ckmc = omain.get("WarehouseName", "")
#             jcdh = omain.get("SNID", "")
#             sfjc = omain.get("sfjc", "")
...
#             #获取业务员path
#             if (ywy):
#                 ywypath = get_path_if_exists(ywy)
#             #获取采购员path
#             if (cgy):
...
#                         wyzd =jcdh + rkrq + name + datetime.now().strftime('%Y-%m-%d %H:%M:%S')+ str(hh) # 生成唯一值
#                         print(wyzd)
#                         oTempData['wyzd'] = wyzd
#                     else:
#                         print('dasdjsalkdj-daskldljsakldjaskldjsakdjaskdjaskldajslkdjaslkdjaslkdjaslkdjasldjasdasjdlasjdlsdjsalkd=======')
...
#                     #                 mm.fsr = name,
#                     #                 mm.xxly = '已进仓通知',
#                     #                 mm.xxnr = name+'的进仓单号'+jcdh+'产品:'+item['ItemNo']+'箱数'+str(item['CartonQty'])+'工厂'+csmc+'的进仓日期:'+ jcrq+'已进仓请查看,请查看,日期:'+ datetime.now().strftime('%Y-%m-%d'),
#                     #                 mm.jsr = gdry1
#                     #                 messages.append(mm)
...
async def fnview_delivery_beforesave(request):
    ...
    try:
        ...
        logger.error('进入产品资料循环-----------------')
        logger.error(f'cpzlList: {cpzlList}')
        for idx, cpzl in enumerate(cpzlList):
            update_item = {'index': idx}
            logger.error(f'处理第{idx}条产品资料: {cpzl}')
            ...
            zcxs1 = cpzl.get('ckxs1', 0)
            logger.error(f'ckxs1 2---: {zcxs1}')
            wyzd = cpzl.get('wyzd', '')
            ckwyzd = cpzl.get('ckwyzd', '')
            logger.error(f'箱数: {zcxs1}, 出库唯一字段: {wyzd}')
            #箱数大于0 以及出库唯一字段不为空
            # zcxs1 > 0 and 
            if wyzd != '' and wyzd != None:
                ...
                if ckwyzd:
                    ...
                    params['wyzd1'] = ckwyzd
                logger.error(f'查询条件: {conditions}, 参数: {params}')
                # 至少需要产品编号或合同编号
                if conditions:
                    ...
                    if len(mmsql) > 0:
                        logger.error(f'查询结果: aaa {mmsql}')
                        i1 = int(mmsql[0]['number1'] or 0)
                        i2 = rkxs - i1
                        logger.error(f'计算结果: rkxs={rkxs}, 已出库箱数={i1}, 在仓箱数={i2}')
                        if i2 > 0:
                            update_item['zcxs1'] = i2
                            i3 = i2 - ckxs
                            cpzl['ckxs1'] = i3
                          
                            logger.error(f'更新在仓箱数: {i3}')
                            if i3 > 0:
                                update_item['kcxs'] = i3
                                update_item['zcxs'] = i3
                                cpzl['zcxs'] = i3
            ...
            # 生成唯一字段
            print("zcxs1zcxs1zcxs1zcxs1zcxs1zcxs1zcxs1zcxs1zcxs1zcxs1zcxs1zcxs1zcxs1zcxs1zcxs1zcxs1zcxs1", zcxs1)
            if zcxs1 != 0:
                jcdh = cpzl.get('jcdh', '')
                rkrq = cpzl.get('rkrq', '')
                ...
                print('11=====================ckwyzdckwyzdckwyzdckwyzd====ckwyzdckwyzdckwyzdckwyzdckwyzd=============',ckwyzd)        
                        ...
                        if parentID == '有':
                            ...
                            update_sql = f"""
                                UPDATE inventorydetail SET 
                                    mtime = :mtime, xgry = :xgry, CartonQty = ifnull(InCartonQty, 0) - {rtn2} - OutCartonQty,
                            ...
                            update_sql = f"""
                                UPDATE inventorydetail SET 
                                    mtime = :mtime, xgry = :xgry, CartonQty = ifnull(InCartonQty, 0) - {ckxs + qckxs} - ReturnCartonQty2,
...
#     """入库操作（根据单据状态判断 insert/update）"""
#     user = request.current_user
#     JSONRes = await request.json()
#     s = Session()
#     try:
#         # 1. 解析参数
#         # purchase_order_no = JSONRes.get('PurchaseOrderNo','')
#         # receipt_rid = JSONRes.get('rid','')
#         # djzt = JSONRes.get('djzt', '')  # 单据状态：未入库/已入库
#         detail_view_data = JSONRes.get('detail_view_data','') 
#         zyh1 = JSONRes.get("zyh1", "")
      
#         products1 = JSONRes.get("products1", "")
#         username = user.username
#         purchase_order_no = products1.get("PurchaseOrderNo", "")
#         receipt_rid = products1.get("rid", "")
#         djzt = products1.get("zt", "")
#         zyh = products1.get("zyh", "")
#         ywry = products1.get("Salesman", "")
#         cgy = products1.get("PurchasingAgent", "")
#         rkrq = products1.get("StorageDate", "")
#         rksj = products1.get("StorageTime", "")
#         jcrq = products1.get("jcrq", "")
#         sdry = products1.get("Operator", "")
#         wxht = products1.get("SalesOrderNo", "")
#         gsmc = products1.get("Exporter", "")
#         ywy = products1.get("Salesman", "")
#         csmc = products1.get("SupplierShortName", "")
#         lhy = products1.get("Collator", "")
#         ckmc = products1.get("WarehouseName", "")
#         jcdh = products1.get("SNID", "")
#         sfjc = products1.get("sfjc", "")
#         gdry = products1.get("gdry", "")
#         yzrq = products1.get("yzrq", "")
#         rkdd = products1.get("rkdd", "")
#         uid = products1.get("uid", "")
#         wyzdzb = products1.get("wyzd", "")
#         khmc = products1.get("khmc", "")
        
     
#         # 1. 去除空格
#         purchase_order_no = purchase_order_no.replace(" ", "")
  
#         # 1. 去除空格
#         purchase_order_no = purchase_order_no.replace(" ", "")
#         receipt_rid = receipt_rid.replace(" ", "")
#         # 2. 判断操作类型
#         # 2. 判断操作类型
#         is_insert = (djzt != '已入库')  # 未入库或空 = 第一次入库（insert）
#         print('is_insert=== ============================================',is_insert,djzt)
...
#                 # 合同总箱数
#                 sql = '''select sum(b.cgxs) as cgxs from cght a,cghtsheet b 
#                          where a.rid = b.pid and a.hthm=:hthm 
#                          and ((b.khhh=:khhh and ifnull(b.khhh, "")<>"") 
#                          or (b.bjhh=:bjhh and ifnull(b.bjhh, "")<>""))'''
#                 amsql = run_sql(sql, {'hthm': purchase_order_no, 'khhh': cpbh, 'bjhh': cpbh})
#                 contract_total = 0
#                 print("amsql = = = = = = = == = = = == = = = = = = == = = = = =",purchase_order_no,cpbh)
#                 if len(amsql) > 0:
#                     contract_total = amsql[0]['cgxs'] or 0 
                
#                 # 历史入库箱数（不含当前入库单）
#                 asql = '''SELECT SUM(b.CartonQty) as total_qty FROM storage a 
#                           INNER JOIN storageline b ON a.rid = b.pid 
#                           WHERE a.PurchaseOrderNo =:PurchaseOrderNo AND b.ItemNo =:ItemNo 
#                           AND a.rid <> :rid'''
...
#         # 4. 执行操作
#         result = {'zyh': '', 'ywbm': '', 'path': '', 'ggxx':'', 'oShipmentsCostsDetail':[]}
        
#         # 获取作业号（第一次入库时）
#         if is_insert and (zyh == '自动编号' or zyh == ''):
#             if zyh1:
#                 result['zyh'] = get_storagezyh(zyh1)
#                 run_sql('update storage set zyh=:zyh where rid=:rid', {"zyh": result['zyh'], "rid": receipt_rid})
        
#         # 获取业务部门、path
#         if ywry:
#             ywbm = get_storageywbm(ywry)
#         if username:
#             result['path'] = get_storageckpath(username)
#         if ywry:
#             ywypath = get_path_if_exists(ywry)
#         if cgy:
#             cgypath = get_path_if_exists(cgy)
        
#         if is_insert and (result['path'] == '' or result['path'] is None):
#             return json_result(-1, '获取用户信息出错！')
...
#             # 超规格检查
#             errors = []
#             ckzd = float(item.get('ckzd') or 0)
#             ckkd = float(item.get('ckkd') or 0)
#             ckgd = float(item.get('ckgd') or 0)
#             ckmz = float(item.get('ckmz') or 0)
#             ckjz = float(item.get('ckjz') or 0)
...
#                 'Exporterc': gsmc,
#                 'Salesmanc': ywy,
#                 'PurchaseOrderNoc': purchase_order_no,
#                 'SupplierShortNamec': csmc,
#                 'PurchasingAgentc': cgy,
#                 'Collatorc': lhy,
#                 'WarehouseNamec': ckmc,
#                 'SNIDc': jcdh,
#                 'sfjcc': sfjc,
...
#                 # ========== 新增（Insert）==========
#                 if item['CartonQty1'] <= 0:
#                     return json_result(-1, f'第{hh}行产品{item["ItemNo"]}的在仓箱数等于0,操作失败！')
                
#                 # 生成唯一字段
#                 wyzd = item.get('wyzd', '')
#                 if not wyzd:
#                     wyzd = jcdh + rkrq + username + datetime.now().strftime('%Y-%m-%d %H:%M:%S') + str(hh)
#                     oTempData['wyzd'] = wyzd
                
#                 kcwyzd = wyzd + rkrq + rksj
                
#                 # 发送通知
#                 for gdry1 in [gdry, ywy]:
#                     if gdry1 and gdry1 != username:
#                         mm = xxck()
#                         mm.rid = get_uuid()
#                         mm.uid = uid
#                         mm.ctime = datetime.now()
...
#                         mm.fsr = username
#                         mm.xxly = '已进仓通知'
#                         mm.xxnr = f'{username}的进仓单号{jcdh}产品:{item["ItemNo"]}箱数{item["CartonQty"]}工厂{csmc}的进仓日期:{jcrq}已进仓请查看,日期:{datetime.now().strftime("%Y-%m-%d")}'
#                         mm.jsr = gdry1
#                         messages.append(mm)
#                 print('end======================================insert=========================================')
#                 # 插入库存明细
...
#                 aa.Salesman = ywy
#                 aa.PurchasingAgent = cgy
#                 aa.PurchaseOrderNo = purchase_order_no
#                 aa.SupplierShortName = csmc
#                 aa.ItemNo = item['ItemNo']
#                 aa.PalletQty = item.get('PalletQty', 0)
#                 aa.CartonQty = item['CartonQty1']
#                 aa.InCartonQty = item['CartonQty']
#                 aa.OutCartonQty = max(0, int(item.get('CartonQty', 0) or 0) - int(item.get('CartonQty1', 0) or 0))
#                 aa.ReturnCartonQty = item.get('ReturnCartonQty', 0)
#                 aa.thyy = item.get('thyy', '')
#                 aa.OuterLength = ckzd
#                 aa.OuterWidth = ckkd
#                 aa.OuterHeight = ckgd
#                 aa.OuterVolume = item.get('cktj', 0)
#                 aa.Volume = item.get('ckztj', 0)
#                 aa.OuterGrossWeight = ckmz
#                 aa.GrossWeight = item.get('ckzmz', 0)
#                 aa.OuterNetWeight = ckjz
#                 aa.WarehousePosition = item.get('WarehousePosition', '')
#                 aa.Collator = lhy
#                 aa.StorageDate = item.get('StorageDate', rkrq)
#                 aa.StorageTime = item.get('StorageTime', rksj)
#                 aa.LotNumber = item.get('LotNumber', '')
#                 aa.SalesOrderNo = wxht
#                 aa.WarehouseName = ckmc
#                 aa.Operator = sdry
#                 aa.rkdd = rkdd
#                 aa.wyzd = wyzd
#                 aa.Memo = item.get('Memo', '')
#                 aa.jcf = item.get('jcf', 0)
#                 aa.wxwyzd = item.get('wxwyzd', '')
#                 aa.cgwyzd = item.get('cgwyzd', '')
#                 aa.OuterLength1 = item.get('OuterLength', 0)
#                 aa.OuterWidth1 = item.get('OuterWidth', 0)
#                 aa.OuterHeight1 = item.get('OuterHeight', 0)
#                 aa.OuterGrossWeight1 = item.get('OuterGrossWeight', 0)
#                 aa.OuterNetWeight1 = item.get('OuterNetWeight', 0)
#                 aa.OuterVolume1 = item.get('OuterVolume', 0)
#                 aa.Volume1 = item.get('Volume', 0)
#                 aa.GrossWeight1 = item.get('GrossWeight', 0)
#                 aa.ywpath = ywypath
#                 aa.cgpath = cgypath
#                 aa.zwmc = item.get('zwmc', '')
#                 aa.UserID = '有'
#                 aa.ReturnCartonQty2 = 0
#                 aa.kcwyzd = kcwyzd
#                 aa.sfyh = item.get('sfyh', '')
#                 aa.ywbm = ywbm
#                 aa.wyzdz = wyzdzb
#                 aa.khmc = khmc
#                 messages1.append(aa)
...
#                 # ========== 更新（Update）==========
#                 # ========== 更新（Update）==========
#                 if item['CartonQty1'] > 0 and item['CartonQty'] > 0:
#                     wyzd = item.get('wyzd', '')
#                     if wyzd:
#                         s.query(inventorydetail).filter(
#                             inventorydetail.wyzd == wyzd
...
#                             'OuterWidth': ckkd,
#                             'OuterHeight': ckgd,
#                             'OuterVolume': item.get('cktj', 0),
#                             'Volume': item.get('ckztj', 0),
#                             'OuterGrossWeight': ckmz,
#                             'OuterNetWeight': ckjz,
#                             'GrossWeight': item.get('ckzmz', 0),
#                             'WarehousePosition': item.get('WarehousePosition', ''),
#                             'Collator': lhy,
...
#                             'Memo': item.get('Memo', ''),
#                             'jcf': item.get('jcf', 0),
#                             "PalletQty": item.get('PalletQty', 0),
#                             "thyy": item.get('thyy', ''),
#                             'sfyh': item.get('sfyh', ''),
#                             'wxwyzd': item.get('wxwyzd', ''),
#                             'cgwyzd': item.get('cgwyzd', ''),
#                             'OuterLength1': item.get('OuterLength', 0),
#                             'OuterWidth1': item.get('OuterWidth', 0),
#                             'OuterHeight1': item.get('OuterHeight', 0),
#                             'OuterGrossWeight1': item.get('OuterGrossWeight', 0),
#                             'OuterNetWeight1': item.get('OuterNetWeight', 0),
#                             'OuterVolume1': item.get('OuterVolume', 0),
#                             'Volume1': item.get('Volume', 0),
#                             'GrossWeight1': item.get('GrossWeight', 0),
#                             'ywpath': ywypath,
#                             'cgpath': cgypath
#                         })
...
#         # 只有第一次入库时才更新单据状态
#         if is_insert:
#             s.execute(f"update storage set zt ='已入库',sfjc='是' where rid='{receipt_rid}'")
...
@require_token
async def view_saier_warehouse_receipt_historical_instock(request):
    """入库操作（根据 inventorydetail 中 wyzd 是否存在判断 insert/update）"""
    user = request.current_user
    JSONRes = await request.json()
    s = Session()
    try:
        ...
        # 2. 先判断整体操作类型（是否有任何产品需要 insert）
        has_insert = False
        # for item in ItemsData:
        #     wyzd_check = item.get('wyzd', '')
        #     if wyzd_check:
        #         # 查询 inventorydetail 是否存在此 wyzd
        #         exists = s.query(inventorydetail.rid).filter(
        #             inventorydetail.wyzd == wyzd_check
        #         ).first()
        #         if not exists:
        #             has_insert = True
        #     else:
        ...
        if 1==1:
            ...
            for item in detail_view_data:
                cpbh = item['ItemNo']
                dqrkxs = float(item.get('CartonQty', 0))
                zpxs = float(item.get('ckxs', 0))
                ...
                if not is_qty_valid or not is_contract_valid:
                    ...
                    if not is_qty_valid and not is_contract_valid:
                        ...
                        error_type = 'CONTRACT_ERROR'
                        has_contract_error = True
                        error_messages.append(f'产品{cpbh}：累计实际进仓数{dqrkxs + historical_qty}大于合同箱数22 {contract_total}！')
        ...
        # 获取作业号（有新增时才需要）
        if has_insert and (zyh == '自动编号' or zyh == ''):
            if zyh1:
                result['zyh'] = get_storagezyh(zyh1)
                run_sql('update storage set zyh=:zyh where rid=:rid', {"zyh": result['zyh'], "rid": receipt_rid})
        ...
        if username:
            ...
            result['path'] = get_storageckpath(username)
            print("username======================================",get_storageckpath(username))
        if ywry:
            ywypath = get_path_if_exists(ywry)
        if cgy:
            cgypath = get_path_if_exists(cgy)
        
        if has_insert and (result['path'] == '' or result['path'] is None):
            return json_result(-1, '获取用户信息出错！')
        
        # 5. 处理明细数据（逐条判断 insert/update）
        hh = 0
        messages = []
        messages1 = []
        ...
        for item in ItemsData:
            ...
            # 判断是新增还是更新（查询 inventorydetail）
            wyzd = item.get('wyzd', '')
            print("dakhksjadhkjsahdjskadhjakshd========")
            ...
            if not wyzd:
             ...
             # 3. 有 wyzd，查询数据库判断是否存在
                exists = s.query(inventorydetail.rid).filter(
                inventorydetail.wyzd == wyzd
                ).first()
                is_insert_item = not exists
            print("is_insert_itemis_insert_itemis_insert_itemis_insert_itemis_insert_itemis_insert_itemis_insert_itemis_insert_item",is_insert_item)
            # 构建返回数据
            oTempData = {
                'rid': item['rid'],
                'khmc': khmc,
