const evt_ckd_FieldChanged = (evt_id, opts) => {
    ...
    if (aFullName.indexOf(field.full_name) != -1) {
        var ckrq = recordset.value('产品资料.出库日期', row);
        var rkrq = recordset.value('产品资料.入库日期', row);
        var cktj = recordset.value('产品资料.出库总体积', row);
        ...
        if ((rkrq != '') && (ckrq != '') && (cktj > 0)) {
            ...
            var days = _getDistanceDays(rkrq, ckrq);
            recordset.val('产品资料.在仓天数', days, row);
            _.http.post('/api/Ravencloud/Select_ckfy', {
                ...
                if (res.code == 1) {
                    console.log('仓库费用')
                    recordset.val('产品资料.综合仓库费用', res.data * 100 / 100, row)
    ...
    if (aFullName.indexOf(field.full_name) != -1) {
        var cght = recordset.value('产品资料.采购合同', row);
        var cpbh = recordset.value('产品资料.产品编号', row);
        var rkrq = recordset.value('产品资料.入库日期', row);
        var rksj = recordset.value('产品资料.入库时间', row);
        var thxs = recordset.value('产品资料.退货箱数', row);
        var cks = recordset.value('产品资料.出库箱数', row);
        ...
        if (cght != '' && cpbh != '' && rkrq != '' && rksj != '' && (thxs > 0 || cks > 0)) {
            if (thzyh != '' && thxs > 0 && cks == 0) {
                ...
                recordset.val('产品资料.出库箱数', thxs, row);
                _.http.post('/api/Ravencloud/Thpd', {
                    ...
                    if (res.code == 1) {
                        console.log('退货匹配成功');
                        recordset.val('产品资料.入库箱数', res.data, row)

                    }
                })
                //出库箱数
                if (recordset.value('产品资料.出库箱数', row) > 0) {
                    if (recordset.value('产品资料.退货箱数', row) > 0) {
                        if (recordset.value('产品资料.出库箱数', row) > recordset.value('产品资料.退货箱数', row)) {
                            _.ui.message.error('出库箱数大于退货箱数,请再输入');
                            recordset.val('产品资料.出库箱数', 0, row)
                        }
                    ...
                    _.http.post('/api/Ravencloud/choose_pd', {
                            ...
                            "rkrq": rkrq,
                            "rksj": rksj,
                            "wyzd1": recordset.value('产品资料.出库唯一字段', row)
                        },

                        true).then(res => {
                        if (res.code == 1) {
                            console.log('退货匹配成功');
                            i2 = (parseFloat(recordset.value('产品资料.入库箱数', row)) || 0) - (parseFloat(res.data) || 0);
                            if (i2 > 0) {
                                recordset.val('产品资料.在仓箱数1', i2, row)
                                i3 = (parseFloat(i2) || 0) - (parseFloat(recordset.value('产品资料.出库箱数', row)) || 0);
                                if (i3 > 0) {
                                    recordset.val('产品资料.库存箱数', i3, row);
                                    recordset.val('产品资料.在仓箱数', i3, row);
                                    recordset.val('产品资料.前出库箱数', res.data, row);
                                } else {
    ...
    if (field.full_name == '出库单.出库日期' || field.full_name == '出库单.入库日期') {
        let t = recordset.tables['产品资料'];
        for (let d of t.view_data) {
            recordset.val('产品资料.出库日期', recordset.val('出库日期'), d);
            recordset.val('产品资料.入库日期', recordset.val('入库日期'), d);
        }
    }
}
...
const fnImportExcel = (recordset) => {
    ...
    input.onchange = async (e) => {
        ...
        try {
            ...
            formData.append('rid', recordset.val('rid'));
            console.log('rid', recordset.val('rid'));
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const cpzlList = cpzlData.map((item, index) => ({
                ...
                jcdh: item.SNID || '',
                cpbh: item.ItemNo || '',
                cght: item.PurchaseOrderNo || '',
                rkrq: item.StorageDate || '',
                rksj: item.StorageTime || '',
                rkxs: parseInt(item.InCartonQty ) || 0,
                ckxs: parseInt(item.OutCartonQty1 ) || 0,
                thxs: parseInt(item.ReturnCartonQty ) || 0,
                zcxs: parseInt(item.CartonQty ) || 0,
                zcxs1: parseInt(item.CartonQty1) || 0,
                wyzd: item.wyzd  || '',
                ckwyzd: item.wyzd1  || '',
                zwmc: item.zwmc  || '',
                csmc: item.SupplierShortName  || '',
                ywy: item.Salesman || '',
                cgy: item.PurchasingAgent || '',
                gdry: item.gdry || '',
                gsmc: item.Exporter  || '',
                wxht: item.SalesOrderNo  || '',
                cw: item.WarehousePosition  || '',
                ph: item.LotNumber || '',
                lhy: item.Collator  || '',
                sdry: item.Operator  || '',
                bz: item.Memo || '',
                thyy: item.thyy || '',
                wxcd: parseFloat(item.OuterLength1 ) || 0,
                wxkd: parseFloat(item.OuterWidth1 ) || 0,
                wxgd: parseFloat(item.OuterHeight1 ) || 0,
                wxmz: parseFloat(item.OuterGrossWeight1 ) || 0,
                wxjz: parseFloat(item.OuterNetWeight1 ) || 0,
                wxtj: parseFloat(item.OuterVolume1 ) || 0,
                ckcd: parseFloat(item.OuterLength ) || 0,
                ckkd: parseFloat(item.OuterWidth ) || 0,
                ckgd: parseFloat(item.OuterHeight ) || 0,
                ckmz: parseFloat(item.OuterGrossWeight ) || 0,
                ckjz: parseFloat(item.OuterNetWeight ) || 0,
                cktj: parseFloat(item.OuterVolume ) || 0,
                ts: item.PalletQty  || '',
                jcf: parseFloat(item.jcf) || 0,
                wxwyzd: item.wxwyzd  || '',
                cgwyzd: item.cgwyzd  || '',
                ywpath: item.ywpath || '',
                cgpath: item.cgpath  || '',
            ...
            _.http.post('/api/ravencloud/delivery_beforesave', {
                ...
                if (res.code == 1) {
                    ...
                    console.log(result.cpzlUpdates);
                    // // 更新产品资料子表
                    if (result.cpzlUpdates && result.cpzlUpdates.length > 0) {
                        const viewData = recordset.tables[cpzlTable].view_data;
                        for (const data of result.cpzlUpdates) {
                            const row = viewData[data.index];
                            if (row) {
                                if (data.wyzd) row['wyzd'] = data.wyzd;
                                if (data.ckwyzd) row['wyzd1'] = data.ckwyzd;
                                if (data.zcxs !== undefined) row['CartonQty1'] = data.zcxs;
                                if (data.zcxs1 !== undefined) row['ckxs1'] = data.zcxs1;
                                if (data.kcxs !== undefined) row['kcxs'] = data.kcxs;
                                if (data.qckxs !== undefined) row['qckxs'] = data.qckxs;
                                if (data.xh) row['ContainerNo'] = data.xh;
                                if (data.fh) row['SealNo'] = data.fh;
                                if (data.ckrq) row['ckrq'] = data.ckrq;
                                row['zyck'] = mainData.zyck;
                                row['WarehouseName'] = mainData.ckmc;
                                // 标记修改
                                if (recordset.tables[cpzlTable].modi_rids.indexOf(row.rid) < 0 &&  recordset.tables[cpzlTable].new_rids.indexOf(row.rid) < 0 ) {
                    ...
                    if (result.jcbhList && result.jcbhList.length > 0) {
                        ...
                        console.log(line_table1.new_rids)
                        line_table1.sync_operate_data();
