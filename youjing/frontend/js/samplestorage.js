...
const samplestorage_out_EditorChildShow = (evt_id, form) => {
    if (form.group.value.name == '出库详情') {
        form.toolbar.add([{
            "name": 'sample_lxm_input_btn',
            "caption": '条码输入',
            "icon": 'any-server-update',
...
_.evts.on([_.evtids.MODULE_EDITOR_GROUP_SHOW], samplestorage_out_EditorChildShow, '样品出库管理')
        ...
        if (recordset.val('入库产品信息.外销唯一') != '' && recordset.val('入库产品信息.采购合同') != '') {
            _.http.post('/api/saier/shipmentsample/rkcp/wxwy/change', {
                ...
                if (res.data && res.data != {}) {
                    ...
                    recordset.val('入库产品信息.起 订 量', Number(res.data.qdl))
...
const shipmentsample_table_after_new = (evt_id, table, recordset) => {
    ...
    if (table.group == '出库详情') {
        ...
        recordset.val('出库详情.人    员', recordset.val('领用人员'));
        recordset.val('出库详情.样品去向', recordset.val('样品去向'));
        recordset.val('出库详情.备    注', recordset.val('备    注'));
    }
    if (table.group == '出库信息') {
...
const sample_in_BtnClick = (evt_id, btn, form) => {
    if (btn.name == 'sample_lxm_input_btn') {
        _.ui.show_dialog('barcode_form', {
            "group": '入库详情',
            "field": '条 形 码',
...
_.evts.on([_.evtids.TOOLBAR_BUTTON_CLICK], sample_in_BtnClick, '样品入库管理')

const sample_out_BtnClick = (evt_id, btn, form) => {
    if (btn.name == 'sample_lxm_input_btn') {
        _.ui.show_dialog('barcode_form', {
            "group": '出库详情',
            "field": '条 形 码',
...
_.evts.on([_.evtids.TOOLBAR_BUTTON_CLICK], sample_out_BtnClick, '样品出库管理')


const sample_disabled_BtnClick = (evt_id, btn, form) => {
    if (btn.name == 'sample_lxm_input_btn') {
        _.ui.show_dialog('barcode_form', {
            "group": '下架详情',
            "field": '条 形 码',
...
_.evts.on([_.evtids.TOOLBAR_BUTTON_CLICK], sample_disabled_BtnClick, '样品下架管理')
