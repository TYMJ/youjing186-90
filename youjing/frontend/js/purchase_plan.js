        ...
        _.http.post('/api/saier/purchase_plan/save/before', {
            ...
            if (cdsb == 0) {
                ...
                if (recordset.val('完成查看') == '锁定') {
                    ...
                    if (jh_data && Object.keys(jh_data).length > 0) {
                ...
                if (xgsb == 0) {
                    ...
                    for (let r of v) {
                        ...
                        if (recordset.val('产品计划') != '是' && recordset.val('产品计划') != '是1') {
                            if (r[recordset.module.field_by_full_name(m + '.产品资料.合同号码').db.name] != recordset.val('外销合同')) {
                                qs.push('请注意第' + i + '条记录合同号' + r[recordset.module.field_by_full_name(m + '.产品资料.合同号码').db.name] + '和外销合同不符');
                            }
                        }
                        ...
                        if (r[recordset.module.field_by_full_name(m + '.产品资料.最低毛利率').db.name] != 0 && r[recordset.module.field_by_full_name(m + '.产品资料.最低毛利率').db.name] > r[recordset.module.field_by_full_name(m + '.产品资料.毛 利 率').db.name]) {
                            ...
                            qs.push('请注意产品:' + r[recordset.module.field_by_full_name(m + '.产品资料.专业货号').db.name] + '的毛利低于' + r[recordset.module.field_by_full_name(m + '.产品资料.最低毛利率').db.name] + '请记得填写成本核算表');
                        } else {
                        ...
                        if (r[recordset.module.field_by_full_name(m + '.产品资料.采购原价').db.name]==undefined || r[recordset.module.field_by_full_name(m + '.产品资料.采购原价').db.name]==null || r[recordset.module.field_by_full_name(m + '.产品资料.采购原价').db.name] == 0) {
                            r[recordset.module.field_by_full_name(m + '.产品资料.采购原价').db.name] = r[recordset.module.field_by_full_name(m + '.产品资料.采购单价').db.name];
                        }
                    ...
                    for (let r of y) {
                        if (r[recordset.module.field_by_full_name(m + '.完成资料.可否重复').db.name] != '可以') {
                            ...
                            if (jz < 0) {
                                jzz.push(r[recordset.module.field_by_full_name(m + '.完成资料.外销唯一字段').db.name] + ';' + r[recordset.module.field_by_full_name(m + '.完成资料.专业厂家id').db.name] + ';' + r[recordset.module.field_by_full_name(m + '.完成资料.下单数量').db.name] + ';' + r[recordset.module.field_by_full_name(m + '.完成资料.采购总额').db.name] + ';' + r[recordset.module.field_by_full_name(m + '.完成资料.下单箱数').db.name]);
                            } else {
                                cywyzdsl = cywyzdsl + 1;
                            }
                        }
                    ...
                    if (recordset.val('问题清单') != '') {
                        // _.ui.message.error(recordset.val('问题清单') + '\r\n' + '请注意以上产品不能下单!!!');
                        _.ui.message_box({
                            title: '异常提示',
                            message: _.vue.h('pre', {
...
const purchase_plan_recordLoad = (evt_id, recordset) => {
    ...
    _.http.post('/api/saier/purchase_plan/load/check', {
        ...
        recordset._list['采购计划.审核申请'] = d.spsq_list
        ...
        _.http.post('/api/saier/purchase_plan/items/invoice/update', {
            ...
            for (let r of d) {
                ...
                if (rid in l) {
                    ...
                    console.log(l[rid])
                    if (l[rid].items) {
                        console.log(l[rid].kpgc)
                        if (l[rid].items.kpgc != '' && l[rid].items.kpgc != null) {
                            r[recordset.module.field_by_full_name(m + '.产品资料.开票工厂').db.name] = l[rid].items.kpgc
                        }
                        r[recordset.module.field_by_full_name(m + '.产品资料.工厂地址').db.name] = l[rid].items.address
                        r[recordset.module.field_by_full_name(m + '.产品资料.所在城市').db.name] = l[rid].items.city1
                        r[recordset.module.field_by_full_name(m + '.产品资料.所在省份').db.name] = l[rid].items.province1
                        r[recordset.module.field_by_full_name(m + '.产品资料.手机号码').db.name] = l[rid].items.sjhm
                        r[recordset.module.field_by_full_name(m + '.产品资料.工厂传真').db.name] = l[rid].items.fax
                        ...
                        r[recordset.module.field_by_full_name(m + '.产品资料.结算方式').db.name] = l[rid].items.jsfs
...
const purchase_plan_field_change = (evt_id, opts) => {
    ...
    if (field.full_name == n + '.产品资料.外销唯一字段' || field.full_name == n + '.产品资料.专业货号') {
        if (recordset.value('产品资料.外销唯一字段', row) != '' && recordset.value('产品资料.专业货号', row) != '') {
            _.http.post('/api/saier/purchase_plan/items/wxwyzd2/change', {
                wyzd: recordset.value('产品资料.外销唯一字段', row),
                zyhh: recordset.value('产品资料.专业货号', row),
            }).then(res => {
    ...
    if (field.full_name == n + '.产品资料.外销唯一字段' && value != '' && value != null) {
        _.http.post('/api/saier/purchase_plan/items/wxwyzd/change', {
            ...
            let cghtsl = 0
            let cgjhsl = d.cgjhsl
            if (recordset.value('产品资料.下单清单', row) != '是') {
                recordset.val('产品资料.下单清单', '是1', row);
                flag = true;
                cghtsl = d.cghtsl;
                cgjhsl = cgjhsl - cghtsl;
            }
            if (cgjhsl > wxhtsl) {
                recordset.val('产品资料.下单数量1', 0, row);
                recordset.val('产品资料.下单箱数1', 0, row);
                recordset.val('产品资料.下单箱数', 0, row);
                recordset.val('产品资料.下单数量', 0, row);
            } else {
                recordset.val('产品资料.下单数量1', wxhtsl - cgjhsl, row);
                if (recordset.value('产品资料.外箱装箱量', row) != 0) {
                    recordset.val('产品资料.下单箱数1', (wxhtsl - cgjhsl) / recordset.val('产品资料.外箱装箱量'), row);
                    recordset.val('产品资料.下单箱数', (wxhtsl - cgjhsl) / recordset.val('产品资料.外箱装箱量'), row);
                }
                recordset.val('产品资料.下单数量', wxhtsl - cgjhsl, row);
            }
            if (recordset.value('产品资料.下单清单', row) != '是') {
                recordset.val('产品资料.下单清单', '是', row);
            }
        }).catch(err => {
            console.log(err)
            _.ui.message.error(err.msg)
    ...
    if (field.full_name == n + '.产品资料.交货日期') {
        if (recordset.value('产品资料.交货日期',row) != '' && recordset.value('产品资料.交货日期',row) != null && recordset.value('产品资料.采购日期',row) != '' && recordset.value('产品资料.采购日期',row) != null) {
            if (recordset.value('产品资料.交货日期',row) < recordset.value('产品资料.采购日期',row) || recordset.value('产品资料.交货日期',row) < new Date().format('yyyy-MM-dd')) {
                _.ui.message.error('请注意交货日期早于今天，或交货日期早于采购日期请重输!')
                recordset.val('产品资料.交货日期', null, row)
            }
        }
    }
    ...
    if (field.full_name == n + '.产品资料.采购单价') {
        if (recordset.value('产品资料.是否赔款', row) == '否') {
            recordset.val('产品资料.采购原价', recordset.value('产品资料.采购单价', row), row);
        }
    }
    if (field.full_name == n + '.产品资料.客户RMB总价' || field.full_name == n + '.产品资料.外销总额' || field.full_name == n + '.产品资料.采购总额' || field.full_name == n + '.产品资料.退 税 率'
        ...
        _.http.post('/api/saier/purchase_plan/items/get/cghl', {
            ...
            let nxkh = res.data.nxkh;
            let sb = res.data.sb;
            if ((recordset.value('产品资料.外销唯一字段',row) != '' && recordset.value('产品资料.采购总额', row) != 0) && (recordset.value('产品资料.外销总额', row) != 0 || recordset.value('产品资料.客户RMB总价', row) != 0)) {
                let cgzj = 0
                let zzsl = 0
                let tsl = 0
                let zzsl1 = 0;
                ...
                let mll = 0;
                let hlcg = 1;
                if (recordset.value('产品资料.采购货币',row) == '' || recordset.value('产品资料.采购货币',row).indexOf('RMB') != -1) {
                    hlcg = 1;
                } else {
                    hlcg = res.data.cghl;
                }
                cgzj = recordset.value('产品资料.采购总额', row) * hlcg + recordset.value('产品资料.辅料总价', row);
                ...
                if (recordset.val('客户判断') != '是') {
                    ...
                    zje = recordset.value('产品资料.外销总额', row);
                } else {
                    hl = 1;
                    zje = recordset.value('产品资料.客户RMB总价', row);
                }
                if (recordset.value('产品资料.佣金比率', row) >= 0 && sb == 0) {
                    recordset.val('产品资料.佣    金', zje * recordset.value('产品资料.佣金比率', row), row);
                } else {
                    if (recordset.value('产品资料.佣金单价', row) >= 0 && sb == 1) {
                        recordset.val('产品资料.佣    金', recordset.value('产品资料.佣金单价', row) * recordset.value('产品资料.下单数量', row), row);
                    }
                }
                if (recordset.value('产品资料.暗佣比率', row) >= 0 && sb == 0) {
                    recordset.val('产品资料.暗佣佣金', zje * recordset.value('产品资料.暗佣比率', row), row);
                } else {
                    if (recordset.value('产品资料.暗佣单价', row) >= 0 && sb == 1) {
                        recordset.val('产品资料.暗佣佣金', recordset.value('产品资料.暗佣单价', row) * recordset.value('产品资料.下单数量', row), row);
                    }
                }
                let mldx = recordset.val('毛利底线');
                if (recordset.val('客户名称') != '') {
                    if (nxkh == '是') {
                        if (recordset.val('客户判断') == '是') {
                            mjdj = recordset.value('产品资料.客户RMB总价', row);
                            sj1 = (mjdj - cgzj - recordset.value('产品资料.代开金额', row)) / zzsl2 * (zzsl1 / 100) + (recordset.value('产品资料.代开点数', row) * recordset.value('产品资料.代开金额', row));
                            mll = (mjdj - recordset.value('产品资料.佣    金', row) - cgzj - recordset.value('产品资料.代开金额', row) - recordset.value('产品资料.暗佣佣金', row)) / (mjdj - recordset.value('产品资料.佣    金', row));
                            ...
                            mjdj = recordset.value('产品资料.外销总额', row) * hl;
                            sj1 = (mjdj - cgzj - recordset.value('产品资料.代开金额', row)) / zzsl2 * (zzsl1 / 100) + (recordset.value('产品资料.代开点数', row) * recordset.value('产品资料.代开金额', row));
                            mll = (mjdj - recordset.value('产品资料.佣    金', row) * hl - cgzj - recordset.value('产品资料.代开金额', row) - recordset.value('产品资料.暗佣佣金', row) * hl) / (mjdj - recordset.value('产品资料.佣    金', row) * hl);
                            recordset.val('产品资料.毛 利 率', mll, row);
                            recordset.val('产品资料.税   金', sj1, row);
                            // if (mldx > 0 && mll < mldx) {
                        ...
                        if (recordset.val('客户判断') != '是') {
                            hl = recordset.val('汇    率');
                            if (recordset.value('产品资料.外销总额', row) > 0 && cgzj > 0) {
                                zzsl = recordset.value('产品资料.增值税率', row);
                                tsl = recordset.value('产品资料.退 税 率', row);
                                let htsl = recordset.value('产品资料.下单数量', row);
                                if (recordset.value('产品资料.外销总额', row) > 1) {
                                    if (tsl == 0) {
                                        zzsl = 0;
                                    }
                                    if ((tsl > 0) || (tsl > zzsl)) {
                                    ...
                                    // && mldx > 0
                                    if (htsl > 0  && hl > 0) {
                                        let hhcb = (cgzj + recordset.value('产品资料.纸卡费用', row) + recordset.value('产品资料.验 货 费', row) + recordset.value('产品资料.包装费用', row) + recordset.value('产品资料.暗佣佣金', row) * hl) / (recordset.value('产品资料.外销总额', row) - recordset.value('产品资料.佣    金', row));
                                        if (zzsl == zzsl1) {
                                            if (tsl == 5) {
                                                a = 1.11;
                                            } else {
                                        ...
                                        console.log('zzsl ', zzsl)
                                        console.log('tsl ', tsl)
                                        console.log('hl ', hl)
                                        console.log('fyl ', fyl)
                                        console.log('zzsl2 ', zzsl2)
                                        console.log('a ', a)
                                        console.log('hhcb ', hhcb)
                                        if (zzsl == 3 && tsl == 3) {
                                            if (1.03 * hl != 0) {
                                                mll = ((1.03 * hl) - (1.03 * hl * fyl) - hhcb) / (1.03 * hl);
                                                hhcb1 = 1.03 * (hl - hl * fyl - hl * mldx);
                            ...
                            if (recordset.value('产品资料.客户RMB总价', row) > 0 && cgzj > 0) {
                                fyl = recordset.value('费 用 率');
                                zzsl = recordset.value('产品资料.增值税率', row);
                                tsl = recordset.value('产品资料.退 税 率', row);
                                mldx = recordset.value('毛利底线', row);
                                let tsl2 = 0;
                                let tsl3 = 0;
                                let mll = 0;
                                if (recordset.value('产品资料.客户RMB总价', row) > 0.0001) {
                                    ...
                                    if (tsl >= 1) {
                                        tsl2 = tsl / 100;
                                        if (zzsl2 != 0) tsl3 = (recordset.value('产品资料.采购总额', row) + recordset.value('产品资料.辅料总价', row)) / zzsl2 * tsl2;
                                    }
                                    if (wxdj2 != 0) mll = (wxdj2 - wxdj2 * fyl - cgzj + tsl3 - recordset.value('产品资料.暗佣佣金', row)) / wxdj2;
                                    recordset.val('产品资料.毛 利 率', mll, row);
                                }
                            }
                        }
    ...
    if (field.full_name == n + '.产品资料.预计船期') {
        ...
        let cgqx = recordset.val('采购合同期限');
        if (recordset.value('产品资料.预计船期', row) != '' && cgqx > 0) {
            let d2 = recordset.value('产品资料.预计船期', row);
            let d = _addDaysDate(d2, -(cgqx + 7));
            recordset.val('产品资料.采购日期', d, row);
        } else {
            recordset.val('产品资料.采购日期', rq, row);
        }
        if (recordset.value('产品资料.预计船期', row) != '' && tjts > 0) {
            let rq1 = _addDaysDate(recordset.value('产品资料.预计船期', row), -tjts);
            recordset.val('产品资料.交货日期', rq1, row);
        } else {
            // recordset.val('产品资料.交货日期', recordset.value('产品资料.预计船期', row));
        }
    }
    if (field.full_name == n + '.产品资料.代开点数') {
        let yjbl = recordset.value('产品资料.代开点数', row);
        recordset.val('产品资料.代开点数', yjbl / 100, row);
    }
    if (field.full_name == n + '.产品资料.下单箱数') {
        if (recordset.val('竞价识别') != '123') {
            if (recordset.value('产品资料.下单箱数', row) > recordset.value('产品资料.箱    数', row)) {
                recordset.val('产品资料.下单箱数', recordset.value('产品资料.箱    数', row), row);
            }
            if ((recordset.value('产品资料.下单箱数', row) > 1) && (recordset.value('产品资料.外箱装箱量', row) > 0) && (recordset.value('产品资料.拼箱标记', row) != '是')) {
                recordset.val('产品资料.下单数量', recordset.value('产品资料.下单箱数', row) * recordset.value('产品资料.外箱装箱量', row), row);
            }
        }
    }
    if (field.full_name == n + '.产品资料.专业厂家id') {
        if (recordset.value('产品资料.专业厂家id', row) == '') {
            recordset.val('产品资料.专业厂家', '待定', row);
            recordset.val('产品资料.工厂评分', 0, row);
            recordset.val('产品资料.有无拜访', '', row);
        } else {
            _.http.post('/api/saier/purchase_plan/items/csbh/change', {
                cs_id: recordset.value('产品资料.专业厂家id', row),
            }).then(res => {
                let d = res.data;
                if (d) {
                    recordset.val('产品资料.工厂评分', d.hzdj1, row);
                    recordset.val('产品资料.有无拜访', d.ywbf, row);
                    recordset.val('产品资料.联 系 人', d.cslxr, row);
                    recordset.val('产品资料.工厂电话', d.phone, row);
                    recordset.val('产品资料.手机号码', d.sjhm, row);
                    recordset.val('产品资料.结算方式', d.jsfs, row);
                    recordset.val('产品资料.工厂传真', d.fax, row);
                    recordset.val('产品资料.所在省份', d.province1, row);
                    recordset.val('产品资料.所在城市', d.city1, row);
                    recordset.val('产品资料.工厂地址', d.address, row);
                    recordset.val('产品资料.专业厂家', d.company_name, row);
                    recordset.val('产品资料.开票工厂', d.kpgc, row);
                } else {
                    recordset.val('产品资料.工厂评分', 0, row);
                    recordset.val('产品资料.专业厂家', '待定', row);
                }
    ...
    if (field.full_name == n + '.产品资料.专业厂家' || field.full_name == n + '.产品资料.专业货号') {
        let sccj = recordset.value('产品资料.生产厂家', row);
        if (sccj == '') {
            sccj = recordset.value('产品资料.专业厂家', row);
        }
        let cpbh = recordset.value('产品资料.产品编号', row);
        if (cpbh == '') {
        ...
        if (cpbh != '' && sccj != '' && sccj != '待定') {
            _.http.post('/api/saier/purchase_plan/items/sccj/change', {
                ...
                let d = res.data;
                if (d) {
                    recordset.val('产品资料.是否开模', d.sfkm, row);
                    recordset.val('产品资料.开模编号', d.kmbh, row);
                }
            }).catch(err => {
                console.log(err)
                _.ui.message.error(err.msg)
    ...
    if (field.full_name == n + '.产品资料.采购人员') {
        let cgry = recordset.value('产品资料.采购人员', row);
        let nbht = recordset.val('宁波合同');
        let ywht = recordset.val('义乌合同');
        let stht = recordset.val('汕头合同');
        let xddd = recordset.value('产品资料.下单地点', row);
        let sccj = recordset.value('产品资料.专业厂家', row);
        let gdry = recordset.value('产品资料.跟单人员', row);
        let wyzd = recordset.value('产品资料.计划唯一', row);
        // if (cgry != '' && cgry != '待定') {
        _.http.post('/api/saier/purchase_plan/items/cgry/change', {
            ...
            if (d.pd && d.pd != '') {
                recordset.val('产品资料.采购人员', '待定', row);
            }
            if (d.sfqr) {
                recordset.val('产品资料.是否确认', d.sfqr, row);
            }
            if (d.ssdq && d.ssdq != '') {
                recordset.val('产品资料.下单地点', d.ssdq, row);
            }
            if (d.path && d.path != '') {
                recordset.val('产品资料.业务', d.path, row);
            }
        }).catch(err => {
            console.log(err)
            _.ui.message.error(err.msg)
    ...
    if (field.full_name == n + '.产品资料.专业货号') {
        ...
        let khhh = recordset.value('产品资料.专业货号', row);
        let khmc = recordset.val('客户名称');
        ...
        if (zyhh != '' && zyhh != null) {
            ...
            if (zyhh != '' && (khmc == '' || khmc.indexOf('BEST PRICE') >= 0 || khmc.indexOf('SIA FP LV') >= 0 || khmc.indexOf('Fix Price General Trading LLC') >= 0)) {
                recordset.val('产品资料.最低毛利率', 0.15, row);
            } else {
                recordset.val('产品资料.最低毛利率', 0, row);
            }
            _.http.post('/api/saier/purchase_plan/items/zyhh/change', {
                ...
                let d = res.data;
                if (d.gdry && d.gdry != '') {
                    recordset.val('产品资料.跟单人员', d.gdry, row);
                }
                if (d.bzyq) {
                    recordset.val('产品资料.包装要求', d.bzyq, row);
                }
                if (d.cgry && d.cgry != '') {
                    recordset.val('产品资料.采购人员', d.cgry, row);
                }
                if (d.zdml && d.zdml > 0) {
                    recordset.val('产品资料.最低毛利率', d.zdml, row);
                }
                if (d.wbcz && d.wbcz != '') {
                    recordset.val('产品资料.设计人员', d.wbcz, row);
                }
            }).catch(err => {
                console.log(err)
                _.ui.message.error(err.msg)
    ...
    if (field.full_name == n + '.产品资料.跟单人员') {
        ...
        recordset.val('产品资料.专业产品编号', gdry, row);
        if (gdry != '' && gdry != '待定') {
            _.http.post('/api/saier/purchase_plan/items/gdry/change', {
                ...
                let d = res.data;
                if (d.path && d.path == '') {
                    recordset.val('产品资料.跟单人员', '待定', row);
                }
                if (d.bm && d.bm != '') {
                    recordset.val('产品资料.跟单部门', d.bm, row);
                }
                if (d.sfqr && d.sfqr != '') {
                    recordset.val('产品资料.是否确认', d.sfqr, row);
                }
            ...
            recordset.val('产品资料.跟单人员', '待定', row);
            recordset.val('产品资料.是否确认', '否', row);
        }
    }

    if (field.full_name == n + '.产品资料.下单数量') {
        let jjsb = recordset.val('竞价识别');
        let xdqd = recordset.value('产品资料.下单清单', row);
        let rid = recordset.value('产品资料.rid', row);
        let xdsl = recordset.value('产品资料.下单数量', row);
        let wxyzd = recordset.value('产品资料.外销唯一字段', row);
        ...
        if (jjsb != '' && xdqd != '是1') {
            _.http.post('/api/saier/purchase_plan/items/xdsl/change', {
                ...
                if (jhhtsl > wxhtsl) {
                    ...
                    let xdsl = recordset.value('产品资料.下单数量', row);
                    let wxrl = recordset.value('产品资料.外箱装箱量', row);
                    let xdsl1 = recordset.value('产品资料.下单数量1', row);
                    let xdxs1 = recordset.value('产品资料.下单箱数1', row);
                    let yjbl = recordset.value('产品资料.佣金比率', row);
                    if (xdsl < xdsl1 && wxrl > 0 && xdsl > 0) {
                        ...
                        if (recordset.value('产品资料.拼箱标记', row) != '是') {
                            recordset.val('产品资料.下单箱数', xdxs, row);
                        }
                        recordset.val('产品资料.客户RMB总价', xdsl * recordset.value('产品资料.赔款RMB', row), row);
                        recordset.val('产品资料.外销总额', xdsl * recordset.value('产品资料.赔款单价', row), row);
                        recordset.val('产品资料.采购总额', xdsl * recordset.value('产品资料.采购单价', row), row);
                        if (recordset.value('客户判断', row) === '是') {
                            recordset.val('产品资料.佣    金', yjbl * recordset.value('产品资料.客户RMB总价', row), row);
                        } else {
                            recordset.val('产品资料.佣    金', yjbl * recordset.value('产品资料.外销总额', row), row);
                        }
                        recordset.val('产品资料.总 毛 重', xdxs * recordset.value('产品资料.毛    重', row), row);
                        recordset.val('产品资料.总 净 重', xdxs * recordset.value('产品资料.净    重', row), row);
                        recordset.val('产品资料.总 体 积', xdxs * recordset.value('产品资料.外箱体积', row), row);
                        // let mz = recordset.val('产品资料.毛    重');
                        // let jz = recordset.val('产品资料.净    重');
                        // let tj = recordset.val('产品资料.外箱体积');
                        recordset.val('产品资料.下单数量1', xdsl, row);
                        if (recordset.value('产品资料.拼箱标记', row) != '是') {
                            recordset.val('产品资料.下单箱数1', xdxs, row);
                        } else {
                            recordset.val('产品资料.下单箱数1', recordset.value('产品资料.下单箱数', row), row);
                        }
                        let ddsl2 = xdxs1 - xdxs;
                        let ddsl = xdsl1 - xdsl;
                        recordset.tables['产品资料'].copy(1).then(res => {
                            recordset.val('产品资料.下单数量1', ddsl, row);
                            recordset.val('产品资料.下单箱数1', ddsl2, row);
                            recordset.val('产品资料.采购日期', null, row);
                            recordset.val('产品资料.采购计划唯一字段', '', row);
                            recordset.val('产品资料.下单箱数', ddsl2, row);
                            recordset.val('产品资料.下单数量', ddsl, row);
                            recordset.val('产品资料.客户RMB总价', ddsl * recordset.value('产品资料.赔款RMB', row), row);
                            recordset.val('产品资料.外销总额', ddsl * recordset.value('产品资料.赔款单价', row), row);
                            recordset.val('产品资料.采购总额', ddsl * recordset.value('产品资料.采购单价', row), row);
                            if (recordset.value('客户判断', row) === '是') {
                                recordset.val('产品资料.佣    金', yjbl * recordset.value('产品资料.客户RMB总价', row), row);
                            } else {
                                recordset.val('产品资料.佣    金', yjbl * recordset.value('产品资料.外销总额', row), row);
                            }
                            recordset.val('产品资料.总 毛 重', ddsl2 * recordset.value('产品资料.毛    重', row), row);
                            recordset.val('产品资料.总 净 重', ddsl2 * recordset.value('产品资料.净    重', row), row);
                            recordset.val('产品资料.总 体 积', ddsl2 * recordset.value('产品资料.外箱体积', row), row);
                        });

                    } else {
                        if (xdsl > xdsl1) {
                            recordset.val('产品资料.下单数量', xdsl1, row);
                        }
                    }
                    if (recordset.value('产品资料.下单箱数', row) === 0) {
                        let xdxs2 = 0;
                        if (wxrl > 0 && xdsl > 0) {
                            if (wxrl < 0.01) {
                                wxrl = 1;
                            ...
                            recordset.val('产品资料.下单箱数', xdxs2, row);
                        }
                    }
                }
            }).catch(err => {
    ...
    if (field.full_name == n + '.审核申请' && value != '') {
        if (recordset.val('审核申请') != username) {
            ...
            if (a1 > 0 || a2 > 0 || a3 > 0 || cywyzdsl > 0) {
                ...
                if (a3 > 0) {
                    _.ui.error_message('行号:' + cpbh2 + '含税但增值税、退税率、中文报关品名,海关编码,报关单位,开票点数没填，或此工厂无开票工厂或货源地,或中文报关品名不能为无！', 5000);
                }
                if (a2 > 0) {
