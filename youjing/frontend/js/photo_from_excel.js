...
_.app.component("photo_from_excel", {
    template: /*html*/ `
    <el-dialog
        v-model="visible"
        :title="_l('更新产品图片')"
        ...
        @closed="do_closed"
        >
        <el-alert :title="_l('第一列放不为空的数据,直到第一列行记录为空时退出')" :closable="false" style="margin-bottom:16px; color:red;" />
    ...
    props: ['id', 'rid', 'module', 'params', 'option','group','kfield','pfield'],
    methods: {
        do_closed() {
        ...
        do_post() {
            ...
            this.data.kfield = this.kfield
            this.data.pfield = this.pfield
            _.http.post('/api/saier/import/photo/from/excel', this.data, {
                headers: {
