...
const customers_recordLoad = (evt_id, recordset) => {
    ...
    let first_loading = recordset.first_loading
    _.http.post('/api/saier/customer/load/check', {
        kh_id: recordset.val('客户编号')
    }).then(res => {
        ...
        if ((position.indexOf('风控') >= 0) || (position.indexOf('财务') >= 0)) {
            ...
            recordset.module.group_by_name('客人采购习惯分析').visible = true;
            recordset.module.group_by_name('业务模式标签').visible = true;
            recordset.module.group_by_name('抬头信息').visible = true;
            recordset.module.group_by_name('历史业务模式标签').visible = true;
            recordset.module.group_by_name('评审信息').visible = true;
            recordset.module.group_by_name('出货汇总').visible = true;
            recordset.module.group_by_name('出货情况').visible = true;
            recordset.module.group_by_name('联系人表').visible = true;
            recordset.module.group_by_name('备注信息').visible = true;
            recordset.module.field_by_full_name(n + '.联 系 人').show();
            recordset.module.field_by_full_name(n + '.内销客户').show();
            recordset.module.field_by_full_name(n + '.电话号码').show();
            recordset.module.field_by_full_name(n + '.最低毛利率').show();
            recordset.module.field_by_full_name(n + '.目标毛利率').show();
            recordset.module.field_by_full_name(n + '.毛 利 比').show();
            recordset.module.field_by_full_name(n + '.年预计金额').show();
            recordset.module.field_by_full_name(n + '.首次合作').show();
            recordset.module.field_by_full_name(n + '.首次接单金额').show();
            recordset.module.field_by_full_name(n + '.首次出货').show();
            recordset.module.field_by_full_name(n + '.首次出货金额').show();
            recordset.module.field_by_full_name(n + '.最新接单').show();
            recordset.module.field_by_full_name(n + '.最新接单金额').show();
            recordset.module.field_by_full_name(n + '.最新出货').show();
            recordset.module.field_by_full_name(n + '.最新出货金额').show();
            recordset.module.field_by_full_name(n + '.Vendor公司').show();
            recordset.module.field_by_full_name(n + '.公海转潜在提交人').show();
            recordset.module.field_by_full_name(n + '.潜在转公海日期').show();

            recordset.module.field_by_full_name(n + '.手机号码').show();
            recordset.module.field_by_full_name(n + '.传真号码').show();
            recordset.module.field_by_full_name(n + '.电子邮件').show();
            recordset.module.field_by_full_name(n + '.公司主页').show();
            recordset.module.field_by_full_name(n + '.详细地址').show();
            recordset.module.field_by_full_name(n + '.邮政编码').show();
            recordset.module.field_by_full_name(n + '.客户来源').show();
            recordset.module.field_by_full_name(n + '.客户密码').show();
            recordset.module.field_by_full_name(n + '.网络查询').show();
            recordset.module.field_by_full_name(n + '.出货情况').show();
            recordset.module.field_by_full_name(n + '.出货汇总').show();
            recordset.module.field_by_full_name(n + '.付款天数').show();
            recordset.module.field_by_full_name(n + '.佣金点数').show();
            recordset.module.field_by_full_name(n + '.开发日期').show();

            recordset.module.field_by_full_name(n + '.抬头代码').show();
            recordset.module.field_by_full_name(n + '.指定货代').show();
            recordset.module.field_by_full_name(n + '.开 证 行').show();
            recordset.module.field_by_full_name(n + '.抬 头 人').show();
            recordset.module.field_by_full_name(n + '.收 货 人').show();
            recordset.module.field_by_full_name(n + '.通 知 人').show();
        }

        if (ry2 == 1 || _.user.username == 'zjnblh' || _.user.username == 'admin') {
        ...
        // }
        // if (first_loading){
        //     recordset.module._.calc_anchor()
        // }
...
const customers_field_change = (evt_id, opts) => {
    ...
    if (field.full_name == n + '.佣金点数') {
        const commissionRate = recordset.val('佣金点数');
        if (commissionRate && commissionRate.toString().includes('0.') && commissionRate < 1) {
            recordset.val('佣金点数', commissionRate * 100);
        }
    }
    ...
    if (field.full_name == n + '.当年主体合作模式') {
        if (recordset.val('当年主体合作模式') == '专业产品模式') {
            console.log('当年主体合作模式 aaaa')
            console.log(recordset.module.field_by_full_name(n + '.当年合作产品品类').disabled)
            recordset.module.field_by_full_name(n + '.当年合作产品品类').disabled = false;
            console.log('当年主体合作模式 cccc')
            console.log(recordset.module.field_by_full_name(n + '.当年合作产品品类').disabled)
        } else {
            recordset.module.field_by_full_name(n + '.当年合作产品品类').disabled = true;
            recordset.val('当年合作产品品类', '');
            console.log('当年主体合作模式 bbbb')
            console.log(recordset.module.field_by_full_name(n + '.当年合作产品品类').disabled)
        }
    }
    if (field.full_name == n + '.业务模式需求趋势') {
        if (recordset.val('业务模式需求趋势') == '专业产品模式') {
            recordset.module.field_by_full_name(n + '.需求趋势产品品类').disabled = false;
        } else {
            recordset.module.field_by_full_name(n + '.需求趋势产品品类').disabled = true;
    ...
    if (field.full_name == n + '.客户眼中的业务模式') {
        if (recordset.val('客户眼中的业务模式') == '专业产品模式') {
            recordset.module.field_by_full_name(n + '.客户眼中模式产品品类').disabled = false;
        } else {
            recordset.module.field_by_full_name(n + '.客户眼中模式产品品类').disabled = true;
...
const customer_before_save = (evt_id, recordset) => {
    return new Promise((resolve, reject) => {
        ...
        if (recordset.val('Vendor ID') == '' && recordset.val('上属公司') != 'BEST PRICE LLC' && recordset.val('客户类型') == '合作客户') {
            _.ui.message.warning('注意客人如果需要锁定报关抬头请填 Vendor')
            // return
        }
...
const customer_after_save = (evt_id, recordset) => {
    ...
    _.http.post("/api/saier/customer/save/after", {
        ...
        khlx: recordset.val('客户类型'),
        fkgl: recordset.val('风控管理'),
        lines: recordset.tables['查看人员清单'].view_data,
        date: date,
