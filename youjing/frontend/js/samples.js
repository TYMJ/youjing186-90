const samples_before_save = (evt_id, recordset) => {
    return new Promise((resolve, reject) => {
        ...
        if (recordset.val('条 形 码') == "" && (recordset.val('存储位置') == "" || recordset.val('一级分类') == "")) {
        ...
        _.http.post('/api/saier/samples/save/before', {
            ...
            if (f) {
                c.sync_operate_data()
                c.modified = true
                recordset.do_re_sum_by_trigger_table('出库信息');
            }
            ...
            if (f) {
                r.sync_operate_data()
                r.modified = true
                recordset.do_re_sum_by_trigger_table('入库产品信息');
            }

            recordset.val('累计出库', cksl)
            recordset.val('累计下架', xjsl)
            if (recordset.val('库存数量') < 0) {
                _.ui.message.error('库存数量不能为负数')
                reject()
                return
            }
            resolve()
...
const samples_after_save = (evt_id, recordset) => {
    if (recordset.val('条 形 码') != "") {
        recordset.module.field_by_full_name('条 形 码').disabled = true;
        recordset.module.field_by_full_name('条形码图').disabled = true;
        recordset.module.field_by_full_name('二维码图').disabled = true;
    }
}
_.evts.on(_.evtids.RECORD_AFTER_SAVE, samples_after_save, '样品管理')
...
const samples_BtnClick = (evt_id, btn, form) => {
    if (btn.name == 'barcode_empty_btn') {
        if (form.recordset.val('条 形 码') !== '') {
            ...
            form.recordset.val('条形码图', '[]');
            form.recordset.val('二维码图', '[]');
        }
    };
    // 当前产品导出（图）
