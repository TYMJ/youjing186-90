...
const purchase_order_field_change = (evt_id, opts) => {
    ...
    if (field.full_name == m + '.产品资料.交货日期') {
        if (
            recordset.val('合同日期') != '' &&
            recordset.value('产品资料.交货日期', row) != '' &&
            recordset.val('合同日期') != null &&
            recordset.value('产品资料.交货日期', row) != null
        ) {
            let date1 = recordset.value('产品资料.交货日期', row)
            let date2 = recordset.val('合同日期')
            if (date1 < date2 || date1 < new Date().format('yyyy-MM-dd')) {
                _.ui.message.error('交货日期不能小于合同日期或小于今天日期！')
    ...
    if (
        ...
        let wxrl = recordset.value('产品资料.内盒装箱量', row)
        let nhwx = recordset.value('产品资料.内盒/外箱', row)
        if (nhwx != '1' && nhwx != '' && nhwx != '0' && nhwx != 0 && nhwx != 1) {
            recordset.val('产品资料.外箱装箱量', Number(wxrl) * Number(nhwx), row)
        }
    }
    if (field.full_name == m + '.产品资料.采购单价') {
        let mrdj = recordset.value('产品资料.默认价格', row)
        let sfpk = recordset.value('产品资料.是否赔款', row)
        let cgyj = recordset.value('产品资料.采购原价', row)
        let htsl = recordset.value('产品资料.合同数量', row)
        if (sfpk != '是') {
            cgyj = recordset.value('产品资料.采购单价', row)
        }
        if (mrdj > 0) {
            recordset.val('产品资料.节省费用', ((mrdj - cgyj) * htsl).toFixed(2), row)
        }
        if (recordset.value('产品资料.节省费用', row) < 0) {
            recordset.val('产品资料.是否确认', '否', row)
        } else {
            recordset.val('产品资料.是否确认', '是', row)
        }
    }
    if (field.full_name == m + '.产品资料.改价原因' && value != '') {
        ...
        if (gjyy == null || gjyy == '') {
            gjyy =
                '产品编号:' +
                recordset.value('产品资料.产品编号', row) +
                '原采购单价:' +
                recordset.value('产品资料.默认价格', row) +
                '现采购单价:' +
                recordset.value('产品资料.采购单价', row) +
                '原因:' +
                recordset.value('产品资料.改价原因', row)
        } else {
            gjyy =
                '\n' +
                '产品编号:' +
    ...
    if (
        ...
        let yjdj = recordset.value('产品资料.佣金单价', row)
        let aydj = recordset.value('产品资料.暗佣单价', row)
        if (yjdj != 0) {
            recordset.val(
                '产品资料.佣    金',
                yjdj * recordset.value('产品资料.合同数量', row),
                row
            )
        } else if (aydj != 0) {
            recordset.val(
                '产品资料.暗佣佣金',
                aydj * recordset.value('产品资料.合同数量', row),
                row
            )
        }
    }
    ...
    if (field.full_name == m + '.产品资料.专业货号') {
        if (
            recordset.value('专业厂家', row) == '' ||
            recordset.value('专业厂家', row) == '待定' ||
            recordset.value('产品资料.专业货号', row) == '' ||
            recordset.value('产品资料.专业货号', row) == null
        ) {
            recordset.module.field_by_full_name(m + '.采购人员').disabled = false
        } else {
            _.http
                .post('/api/saier/purchase_order/zyhh/change', {
                    sccj: recordset.value('专业厂家'),
                    zyhh: recordset.value('产品资料.专业货号', row),
                })
                .then((res) => {
                    if (res.data.cgy != null && res.data.cgy != '') {
                        recordset.val('采购人员', res.data.cgy, row)
                    } else {
                        recordset.module.field_by_full_name(m + '.采购人员').disabled = false
                    }
                })
    ...
    if (field.full_name == m + '.产品资料.主产品货号' && value != '') {
        _.http
            .post('/api/saier/purchase_order/zcphh/change', {
                zyhh: recordset.value('产品资料.主产品货号', row),
            })
            .then((res) => {
                let d = res.data
                if (d && Number(d) < 0.00001) recordset.val('产品资料.主产品货号', '', row)
            })
            .catch((err) => {
                console.log(err)
                _.ui.message.error(err.msg)
    ...
    if (
        ...
        _.http
            .post('/api/saier/purchase_order/khhh/change', {
                zyhh: recordset.value('产品资料.专业货号', row),
                khhh: recordset.value('产品资料.客户货号', row),
                khbh: recordset.value('客户编号'),
            })
            .then((res) => {
                let d = res.data
                if (d && d != '') recordset.val('产品资料.专业货号', d, row)
            })
            .catch((err) => {
                console.log(err)
                _.ui.message.error(err.msg)
    ...
    if (
        ...
        let htsl = recordset.value('产品资料.合同数量', row)
        let fllj = recordset.value('产品资料.辅料价格', row)
        let cgzj = Math.floor(cgdj * htsl * 100) / 100
        recordset.val('产品资料.总 金 额', cgzj, row)
        recordset.val('产品资料.辅料总价', Math.floor(fllj * htsl * 100) / 100, row)
        if (recordset.value('产品资料.是否赔款', row) != '是') {
            recordset.val('产品资料.采购原价', cgdj, row)
        }
        let cgyj = recordset.value('产品资料.采购原价', row)
        if (cgyj == 0) {
        ...
        if (
            cgzj != 0 &&
            recordset.value('产品资料.单证锁定', row) == '' &&
            recordset.value('是否核准', row) == '通过' &&
            recordset.value('下单申请', row) == username
        ) {
            _.http
                ...
                .then((res) => {
                    let d = res.data
                    if (d) {
                        recordset.val('产品资料.外销单价', d.wxdj, row)
                        recordset.val('产品资料.赔款单价', d.Twxdj, row)
                        recordset.val('客户RMB单价', d.mjdj1, row)
                        recordset.val('赔款RMB', d.pkRMB, row)
                        recordset.val('暗佣比率', d.aybl, row)
                        recordset.val('暗佣单价', d.asl6, row)
                        recordset.val('佣金比率', d.yjbl, row)
                        recordset.val('佣金单价', d.sl6, row)
                    }
                })
                .catch((err) => {
                    console.log(err)
    ...
    if (field.full_name == m + '.产品资料.专业货号') {
        let hthm = recordset.value('合同号码', row)
        hthm = hthm.toUpperCase()
        let wxht = recordset.value('外销合同', row)
        ...
        if (recordset.value('产品资料.专业货号', row) != '') {
            ...
            if (
                cx1z.toUpperCase() == 'L' &&
                recordset.value('产品资料.主产品货号', row) == ''
            ) {
            ...
            recordset.val(
                '产品资料.专业货号',
                recordset.val('产品资料.专业货号', row).trim(),
                row
            )
        }
        ...
        if (
            recordset.value('产品资料.专业货号', row) != '' &&
            (recordset.value('是否辅料', row) == '是' ||
                hthm.indexOf('AMZ') != -1 ||
                wxht.indexOf('AMZ') != -1)
        ) {
            recordset.val('产品资料.专业货号1', recordset.value('产品资料.专业货号', row), row)
            let khbh = recordset.value('客户编号')
            let zyhh = recordset.value('产品资料.专业货号', row)
            if (recordset.value('部门识别', row) == '1') {
                _.http
                    ...
                    .then((res) => {
                        ...
                        if (!d) {
                            ...
                            recordset.val('产品资料.客户货号', d.krhh, row)
                            recordset.val('产品资料.客人条码', d.krtm, row)
                            if (d.fljg) recordset.val('产品资料.辅料价格', d.fljg, row)
                            recordset.val('产品资料.开票点数', d.ljrk, row)
                            recordset.val('产品资料.产品规格', d.cpggz, row)
                            recordset.val('产品资料.规格英语', d.cpgg, row)
                            recordset.val('产品资料.可思达货号', d.ksdhh, row)
                            recordset.val('产品资料.颜    色', d.yse, row)
                            recordset.val('产品资料.货币代码', d.cghbdm, row)
                            recordset.val('产品资料.颜色英文', d.ysez, row)
                            recordset.val('产品资料.中文品名', d.zwpm, row)
                            recordset.val('产品资料.英文品名', d.ywpm, row)
                            recordset.val('产品资料.计量单位', d.jldw, row)
                            recordset.val('产品资料.中文计量单位', d.zwdw, row)
                            recordset.val('产品资料.采购单价', d.cgdj, row)
                            recordset.val('产品资料.比  值', d.hhcb, row)
                            recordset.val('产品资料.包装单位', d.bzdw, row)
                            recordset.val('产品资料.包装长度', d.bzcd, row)
                            recordset.val('产品资料.包装宽度', d.bzkd, row)
                            recordset.val('产品资料.包装高度', d.bzgd, row)
                            recordset.val('产品资料.内盒/外箱', d.nhwx, row)
                            recordset.val('产品资料.外箱体积', d.bztj, row)
                            recordset.val('产品资料.中文包装', d.zhwbzh, row)
                            recordset.val('产品资料.英文包装', d.bzhfsh, row)
                            recordset.val('产品资料.毛    重', d.mxmz, row)
                            recordset.val('产品资料.净    重', d.mxjz, row)
                            recordset.val('产品资料.产品尺寸', d.chpchc, row)
                            recordset.val('产品资料.中文说明', d.cpsm, row)
                            recordset.val('产品资料.英文说明', d.ywsm, row)
                            recordset.val('产品资料.中文尺寸', d.zwcc, row)
                            recordset.val('产品资料.英文尺寸', d.ywcc, row)
                            recordset.val('产品资料.款  式', d.ks, row)
                            ...
                            recordset.val('产品资料.规格外语', d.ggwy, row)
                            recordset.val('产品资料.材质外语', d.czwy, row)
                            recordset.val('产品资料.颜色外语', d.yswy, row)
                            recordset.val('产品资料.外语说明', d.wysm, row)
                            recordset.val('产品资料.单据品名', d.djpm, row)
                            recordset.val('产品资料.单据品名英', d.djpmy, row)
                            recordset.val('产品资料.单据品名外', d.djpmw, row)
                            recordset.val('产品资料.客人CODE', d.krcode, row)
                            recordset.val('产品资料.材  质', d.caiziz, row)
                            recordset.val('产品资料.中文报关品名', d.bgpm, row)
                            recordset.val('产品资料.海关编码', d.hgbm, row)
                            recordset.val('产品资料.备    注', d.bz3, row)
                            recordset.val('产品资料.内盒装箱量', d.nhrl, row)
                            recordset.val('产品资料.外箱装箱量', d.bzrl, row)
                            recordset.val('产品资料.增值税率', d.zzsl, row)
                            recordset.val('产品资料.退 税 率', d.tsl, row)
                            recordset.val('产品资料.报关单位', d.bgjldw, row)
                            recordset.val('产品资料.产品来源', d.topcz, row)
                            recordset.val('产品资料.专业产品编号', d.cpbh, row)
                            recordset.val('产品资料.工厂货号', d.gchh1, row)
                        }
                    })
                    .catch((err) => {
                        console.log(err)
                ...
                recordset.val('产品资料.货号备注', zyhh, row)
            }
        }
    }
    if (
        ...
        let wxwyzd = recordset.value('产品资料.外销唯一字段', row)
        let cgwyzd = recordset.value('产品资料.唯一字段', row)
        let htsl = 0
        if ((xs > 0 || recordset.value('产品资料.合同数量', row) > 0) && wxzl > 0) {
            if (recordset.value('产品资料.拼箱标记', row) != '是') {
                htsl =
                    recordset.value('产品资料.外箱装箱量', row) *
                    recordset.value('产品资料.箱    数', row)
            } else {
                htsl = recordset.value('产品资料.合同数量', row)
            }
            _.http
                ...
                .then((res) => {
                    ...
                    if (
                        ...
                        recordset.val(
                            '产品资料.剩余数量',
                            recordset.value('产品资料.合同数量', row), row
                        )
                        recordset.val('产品资料.出货数量', 0, row)
                        recordset.val('产品资料.出货合计', 0, row)
                    } else {
                        if (saved == 1) {
                            _.ui.message.error(
                                '此合同总体下单数大于外销下单数请核实。请输入原箱数！' + cgxs,
                            )
                            recordset.val('产品资料.外箱装箱量', wxrl, row)
                            recordset.val('产品资料.合同数量', 0, row)
                            recordset.val('产品资料.箱    数', 0, row)
                        } else {
                ...
                .catch((r) => {
                    ...
                    recordset.val('产品资料.合同数量', chsl, row)
                    recordset.val('产品资料.箱    数', Math.ceil(chsl / wxzl), row)
                })
        }
    }
            ...
            _.http
                ...
                .then((res) => {
                    ...
                    if (d && (sdpd == 1 || ytsb == 1)) {
                        ...
                        let cgjg1 = round(recordset.value('产品资料.采购单价', row), 3)
                        let kpgc1 = recordset.value('产品资料.开票工厂', row)
                        let zzjgdm1 = recordset.value('产品资料.组织机构代码', row)
                        let kplxr1 = recordset.value('产品资料.开票联系人', row)
                        let kpdh1 = recordset.value('产品资料.开票电话', row)
                        let cpgg1 = recordset.value('产品资料.产品规格', row)
                        let sccj11 = recordset.value('产品资料.专业厂家', row)
                        let jsfs1 = recordset.value('产品资料.结算方式', row)
                        if (
                            kpgc != kpgc1 ||
                            zzjgdm != zzjgdm1 ||
                            kplxr != kplxr1 ||
                            ...
                            recordset.val('产品资料.采购单价', cgjg, row)
                            recordset.val('产品资料.内盒装箱量', nhrl, row)
                            recordset.val('产品资料.内盒/外箱', nhwx, row)
                            recordset.val('产品资料.外箱装箱量', wxrl, row)
                            recordset.val('产品资料.合同数量', cgsl, row)
                            recordset.val('产品资料.箱    数', cgxs, row)
                            recordset.val('产品资料.开票工厂', kpgc, row)
                            recordset.val('产品资料.组织机构代码', zzjgdm, row)
                            recordset.val('产品资料.开票联系人', kplxr, row)
                            recordset.val('产品资料.开票电话', kpdh, row)
                            recordset.val('产品资料.产品规格', cpgg, row)
                            recordset.val('产品资料.专业厂家', sccj1, row)
                            recordset.val('产品资料.结算方式', jsfs, row)
                        }
                    } else {
                        recordset.val('产品资料.单证锁定', '', row)
                    }
                })
                .catch((r) => {
                    console.log(r)
    ...
    if (fields.indexOf(field.full_name) !== -1) {
        ...
        if (recordset.val('客户判断') != '是') {
            zje =
                recordset.value('产品资料.赔款单价', row) + recordset.value('产品资料.合同数量', row)
        } else {
            zje =
                recordset.value('产品资料.赔款RMB', row) + recordset.value('产品资料.合同数量', row)
        }
        ...
        if (htje != 0) {
            ...
            _.http
                .post('/api/saier/purchase_order/item/zje/change', {
                    khmc: recordset.val('客户名称'),
                    cgbz: recordset.value('产品资料.货币代码', row),
                })
                .then((res) => {
                    let d = res.data
                    let hlcg = d.cghl
                    ...
                    let zjl = d.zjl
                    let cgzj =
                        recordset.value('产品资料.总 金 额', row) * hlcg +
                        recordset.value('产品资料.辅料总价', row)
                    ...
                    let mldx = recordset.value('毛利底线', row)
                    let htsl = recordset.value('产品资料.合同数量', row)
                    let nxkh = d.nxkh
                    if (recordset.val('客户名称') != '') {
                        if (recordset.value('产品资料.佣金比率', row) >= 0 && sb == 0) {
                            recordset.val(
                                '产品资料.佣    金',
                                zje * recordset.value('产品资料.佣金比率', row), row
                            )
                        } else {
                    ...
                    if (nxkh === '是') {
                        if (recordset.val('客户判断') === '是') {
                            let mjdj =
                                recordset.value('产品资料.赔款RMB', row) *
                                recordset.value('产品资料.合同数量', row)
                            let sj1 =
                                ((mjdj - cgzj - recordset.value('产品资料.代开金额', row)) / zzsl2) *
                                (zzsl1 / 100) +
                                recordset.value('产品资料.代开点数', row) *
                                recordset.value('产品资料.代开金额', row)
                            let mll =
                                (mjdj -
                                    recordset.value('产品资料.佣    金', row) -
                                    cgzj -
                                    recordset.value('产品资料.代开金额', row) -
                                    recordset.value('产品资料.暗佣佣金', row)) /
                                (mjdj - recordset.value('产品资料.佣    金', row))
                            recordset.val('产品资料.毛 利 率', mll, row)
                            recordset.val('产品资料.税   金', sj1, row)
                        } else {
                            let mjdj =
                                recordset.value('产品资料.赔款单价', row) *
                                recordset.value('产品资料.合同数量', row) *
                                hl
                            if (recordset.value('产品资料.佣    金', row) === 0) {
                                recordset.val(
                                    '产品资料.佣    金',
                                    mjdj * recordset.value('产品资料.佣金比率', row),
                                    row
                                )
                            }
                            let sj1 =
                            ...
                            let mll =
                                (mjdj -
                                    recordset.value('产品资料.佣    金', row) * hl -
                                    cgzj -
                                    recordset.value('产品资料.代开金额', row) -
                                    recordset.value('产品资料.暗佣佣金', row) * hl) /
                                mjdj
                            recordset.val('产品资料.毛 利 率', mll, row)
                            recordset.val('产品资料.税   金', sj1, row)
                        }
                        ...
                        if (
                            (recordset.value('产品资料.赔款单价', row) > 0 ||
                                recordset.value('产品资料.赔款RMB', row) > 0) &&
                            cgzj > 0
                        ) {
                            ...
                            if (tsl > 0 || tsl > zzsl) {
                                if (tsl === 3) {
                                    zzsl = 3
                                    recordset.value('产品资料.增值税率', zzsl, row)
                                }
                                if (tsl > 3) {
                                    zzsl = zzsl1
                                    recordset.value('产品资料.增值税率', zzsl, row)
                            ...
                            if (htsl > 0 && mldx > 0 && hl > 0) {
                                ...
                                if (recordset.val('客户判断') !== '是') {
                                    hhcb =
                                        (cgzj +
                                            recordset.value('产品资料.纸卡费用', row) +
                                            recordset.value('产品资料.暗佣佣金', row) * hl) /
                                        (htsl * recordset.value('产品资料.赔款单价', row) -
                                            recordset.value('产品资料.佣    金', row))
                                } else {
                                    hhcb =
                                        (cgzj +
                                            recordset.value('产品资料.纸卡费用', row) +
                                            recordset.value('产品资料.暗佣佣金', row)) /
                                        (htsl * recordset.value('产品资料.赔款RMB', row) -
                                            recordset.value('产品资料.佣    金', row))
                                }
                                if (zzsl === zzsl1) {
                                    if (tsl === 5) {
    ...
    _.http
        ...
        .then((res) => {
            ...
            if (recordset.val('wf_status') == 1) {
                ...
                if (recordset.val('采购人员') != _.user.username) return
